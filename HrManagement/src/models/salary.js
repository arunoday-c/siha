    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const previous_month =
          parseInt(input.month) == 1 ? 12 : parseInt(input.month) - 1;
        const previous_month_year =
          parseInt(input.month) == 1 ? parseInt(input.year) - 1 : input.year;
        ...
        _mysql
          ...
          .then((employee_data) => {
            ...
            empResult = _.chain(empResult)
              .groupBy((g) => g.employee_id)
              .map((details) => {
                const firstRecord = _.head(details);
                const record =
                  details.length === 1 &&
                  firstRecord.salary_processed === "Y" &&
                  firstRecord.salary_type === "LS"
                    ? firstRecord
            ...
            empResult.map((o) => {
              ...
              if (
                o.salary_processed == "N" &&
                (o.salary_type == "NS" || o.salary_type == "FS")
              ) {
                _salaryHeader_id.push(o.hims_f_salary_id);
                  ...
                  if (req.connection == null) {
                    _mysql.commitTransaction(() => {
                      ...
                      req.records = {
                        invalid_input: true,
                        message:
                          "Previous month salary payment is pending for the assigned filter.",
                      };
                      next();
                    });
                  } else {
                ...
                new Promise((resolve, reject) => {
                  try {
                    ...
                    for (let i = 0; i < empResult.length; i++) {
                      ...
                      const _earlyJoin = _.filter(results[22], (f) => {
                        return f.employee_id == empResult[i]["employee_id"];
                      });
                      ...
                      getEarningComponents({
                        ...
                        .then((earningOutput) => {
                          ...
                          const ls_ded_applied = _.filter(results[18], (f) => {
                            return f.employee_id == empResult[i]["employee_id"];
                          });
                          ...
                          getDeductionComponents({
                            ...
                            getContrubutionsComponents({
                              ...
                              const _LS_loan_deduct = _.filter(
                                results[21],
                                (f) => {
                                  return (
                                    f.employee_id == empResult[i]["employee_id"]
      ...
      _mysql
        .executeQueryWithTransaction({
          query:
            ...
            AL.from_normal_salary, AL.hims_f_employee_annual_leave_id, LA.employee_joined, S.salary_type,S.month,S.salary_date,S.hospital_id from hims_f_salary S \
          ...
          if (salary_process.length > 0) {
            ...
            let _salary_data = salary_process.filter((f) => {
              ...
              if (
                ...
                return false;
              if (f.employee_joined == "Y") return f;
              return (
                f.from_normal_salary === "Y" ||
                f.hims_f_employee_annual_leave_id === null ||
                (f.last_salary_process_date !== null
...
then round((monthly_accrual_days / 30) * (datediff(E.exit_date, '${month_start}') + 1), 2)
      ...
      strQuery +=
        "select employee_id from hims_f_leave_salary_accrual_detail where employee_id in (?) and year=? and month=?; \
        SELECT * FROM hims_f_leave_salary_accrual_header where year=? and month=? ORDER BY hims_f_leave_salary_accrual_header_id DESC LIMIT 1 ;";
      ...
      _mysql
        ...
        .then((leave_accrual_data) => {
          ...
          const header_accrual_exit = leave_accrual_data[3];
          ...
          if (leave_accrual_detail.length > 0) {
            ...
            for (let i = 0; i < accrual_exit.length; i++) {
              const salary_pay_acc = leave_salary_accrual_detail.find(
                (f) => f.employee_id === accrual_exit[i].employee_id
              );
            ...
            if (leave_salary_accrual_detail.length > 0) {
              InsertLeaveSalaryAccrual({
                req: req,
                inputParam: inputParam,
                leave_salary_accrual_detail: leave_salary_accrual_detail,
                header_accrual_exit: header_accrual_exit,
                annual_leave_data: annual_leave_data,
                _mysql: _mysql,
                next: next,
                decimal_places: req.userIdentity.decimal_places,
                isSingleEmployee:
                  inputParam.employee_id.length === 1 ? true : false,
              })
                .then((Employee_Leave_Salary) => {
                  // _mysql
            ...
            if (
              ...
              select head_id, child_id from hims_d_earning_deduction where component_category='E' and component_type='EOS';",
                ...
                .then((result) => {
                  ...
                  //monthname(concat('1999-',month,'-01'))
                  _mysql
                    .executeQueryWithTransaction({
                                ...
                                insert_finance_detail.push(
                                  ...
                                  ...loan_data,
                                  {
                                    day_end_header_id:
                                      per_salary.finance_day_end_header_id,
                                    payment_date:
                                      employee_barnch.transaction_date,
...
function InsertLeaveSalaryAccrual(options) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const header_accrual_exit = options.header_accrual_exit;
      ...
      if (header_accrual_exit.length > 0) {
        ...
        const total_leave_salary =
          _.sumBy(leave_salary_accrual_detail, (s) => {
            return parseFloat(s.leave_salary);
          }) + parseFloat(header_data_exit.total_leave_salary);

        const total_airfare_amount =
          _.sumBy(leave_salary_accrual_detail, (s) => {
            return parseFloat(s.airfare_amount);
        ...
        _mysql
          .executeQueryWithTransaction({
            query:
              "UPDATE `hims_f_leave_salary_accrual_header` set total_leave_salary=?, total_airfare_amount=? where hims_f_leave_salary_accrual_header_id=?",
            values: [
              total_leave_salary,
              total_airfare_amount,
              header_data_exit.hims_f_leave_salary_accrual_header_id,
            ],
            printQuery: true,
          })
          .then((accrual_header) => {
...
function InsertEmployeeLeaveSalary(options) {
  return new Promise((resolve, reject) => {
    try {
      ...
      let employee_ids = [];
      leave_salary_accrual_detail.map((o) => {
        ...
        .executeQueryWithTransaction({
          query:
            "select hims_f_employee_leave_salary_header_id,employee_id,leave_days,leave_salary_amount, \
                ...
                from hims_f_employee_leave_salary_header where employee_id in (?);\
                select employee_id, hims_f_employee_monthly_leave_id, close_balance, accumulated_leaves, projected_applied_leaves \
                from hims_f_employee_monthly_leave where year = ? and employee_id in (?) and leave_id=?;\
                select hims_f_employee_leave_salary_detail_id, hims_f_employee_leave_salary_header_id, employee_id from hims_f_employee_leave_salary_header H \
                ...
                where H.employee_id in (?) and D.year=? and D.month=?;",
          values: [
            employee_ids,
            leave_salary_accrual_detail[0].year,
            employee_ids,
            annual_leave_data[0].hims_d_leave_id,
            employee_ids,
            leave_salary_accrual_detail[0].year,
            leave_salary_accrual_detail[0].month,
          ],
          printQuery: true,
        })
        .then((leave_salary_data) => {
          const employee_leave_salary = leave_salary_data[0];
          const monthly_leave = leave_salary_data[1];
          const detail_exisits = leave_salary_data[2];

          // console.log("leave_salary_accrual_detail", leave_salary_accrual_detail)
          // if (employee_leave_salary.length > 0) {
          for (let i = 0; i < leave_salary_accrual_detail.length; i++) {
            const employee_leave_salary_header = employee_leave_salary.find(
              (f) =>
                f.employee_id === leave_salary_accrual_detail[i].employee_id
            );
            ...
            // }
            // console.log("1")
            const employee_monthly_leave = monthly_leave.find(
              (f) =>
                f.employee_id === leave_salary_accrual_detail[i].employee_id
            );
            const employee_detail_exisits = detail_exisits.find(
              (f) =>
                f.employee_id === leave_salary_accrual_detail[i].employee_id
            );

            if (employee_leave_salary_header !== undefined) {
              if (employee_detail_exisits === undefined) {
                const leave_days =
                  parseFloat(employee_leave_salary_header.leave_days) +
                  parseFloat(leave_salary_accrual_detail[i].leave_days);
                const leave_salary_amount = utilities.decimalPoints(
                  parseFloat(employee_leave_salary_header.leave_salary_amount) +
                    parseFloat(leave_salary_accrual_detail[i].leave_salary),
                  decimal_places
                );
                const airticket_amount = utilities.decimalPoints(
                  parseFloat(employee_leave_salary_header.airticket_amount) +
                    parseFloat(leave_salary_accrual_detail[i].airfare_amount),
                  decimal_places
                );
                const balance_leave_days = utilities.decimalPoints(
                  parseFloat(employee_leave_salary_header.balance_leave_days) +
                    parseFloat(leave_salary_accrual_detail[i].leave_days),
                  decimal_places
                );
                const balance_leave_salary_amount = utilities.decimalPoints(
                  parseFloat(
                    employee_leave_salary_header.balance_leave_salary_amount
                  ) + parseFloat(leave_salary_accrual_detail[i].leave_salary),
                  decimal_places
                );
                const balance_airticket_amount = utilities.decimalPoints(
                  parseFloat(
                    employee_leave_salary_header.balance_airticket_amount
                  ) + parseFloat(leave_salary_accrual_detail[i].airfare_amount),
                  decimal_places
                );

                const airfare_months =
                  parseFloat(employee_leave_salary_header.airfare_months) + 1;

                let monthly_close_balance = parseFloat(
                  employee_monthly_leave.close_balance
                );

                let projected_applied_leaves = parseFloat(
                  employee_monthly_leave.projected_applied_leaves
                );
                let accumulated_leaves = parseFloat(
                  employee_monthly_leave.accumulated_leaves
                );
                ...
                strQry += mysql.format(
                  ...
                  [
                    ...
                    airfare_months,
                    employee_leave_salary_header.hims_f_employee_leave_salary_header_id,
                    monthly_close_balance,
                    projected_applied_leaves,
                    accumulated_leaves,
                    employee_monthly_leave.hims_f_employee_monthly_leave_id,
                    employee_leave_salary_header.hims_f_employee_leave_salary_header_id,
                    leave_salary_accrual_detail[i].leave_days,
                ...
                .executeQueryWithTransaction({
                  query:
                    "INSERT INTO `hims_f_employee_leave_salary_header`  (`year`,`employee_id`,`leave_days`,\
                          ...
                          `balance_airticket_amount`,`airfare_months`,`utilized_leave_days`, `utilized_leave_salary_amount`, \
                          `utilized_airticket_amount`) VALUE(?,?,?,?,?,?,?,?,?,?,?,?)",
                  values: [
                    leave_salary_accrual_detail[i].year,
                  ...
                  _mysql
                    ...
                    .then((leave_detail) => {
                      let monthly_close_balance =
                        employee_monthly_leave.close_balance === null
                          ? 0
                          : parseFloat(employee_monthly_leave.close_balance);
            ...
            // console.log("Length", leave_salary_accrual_detail.length)
...
function InsertEmployeeLeaveSalary_Old(options) {
  return new Promise((resolve, reject) => {
    try {
      let leave_salary_accrual_detail = options.leave_salary_accrual_detail;
      let _mysql = options._mysql;
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (options.over_time.length > 0) {
        ...
        let Noof_Working_Hours =
          hrms_option[0].working_hour_from === "E"
            ? over_time["standard_work_hours"]
            : parseFloat(hrms_option[0].standard_working_hours) -
              parseFloat(hrms_option[0].standard_break_hours);
...
function getEarningComponents(options) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const cal_total_days =
        hrms_option[0].salary_calendar === "F"
          ? hrms_option[0].salary_calendar_fixed_days
          : empResult["total_days"];
        ...
        if (obj.calculation_type == "F") {
          if (
            ...
            current_earning_per_day_salary = parseFloat(
              obj["amount"] / cal_total_days
            );

            current_earning_amt =
              current_earning_per_day_salary *
              (parseFloat(empResult["total_paid_days"]) - early_join_days);
          }
          ...
          if (leave_salary == null || leave_salary == undefined) {
            ...
            const first_month_leave_period =
              parseFloat(empResult["paid_leave"]) -
              parseFloat(input.first_month_leave_period);

            leave_salary_days = parseFloat(empResult["total_paid_days"]);
            // leave_salary_days =
...
function getDeductionComponents(options) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const ls_ded_applied = options.ls_ded_applied;
...
function getContrubutionsComponents(options) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const decimal_places = options.decimal_places;
      const ls_applied = options.ls_applied;
      const ls_con_applied = options.ls_con_applied;
          ...
          .then((result) => {
            ...
            if (result[0].length > 0) {
              result[0].forEach((project) => {
                ...
                basic_cost = utilities.decimalPoints(
                  basic_cost,
                  decimal_places
                );
                ot_cost = utilities.decimalPoints(ot_cost, decimal_places);
                wot_cost = utilities.decimalPoints(wot_cost, decimal_places);
...
export function makeSalaryUnfinalized(req, res, next) {
  const _mysql = new algaehMysql();
  try {
    // UPADTE hims_f_salary SET salary_processed='N' where hims_f_salary_id=?;\values: [inputParam.hims_f_salary_id],
    const inputParam = req.body;
    _mysql
      .executeQueryWithTransaction({
      ...
      .then((org_data) => {
        console.log("org_data", org_data[0]);
        // consol.log("org_data", org_data[0]);
        if (
          org_data[0]["product_type"] == "HIMS_ERP" ||
          org_data[0]["product_type"] == "FINANCE_ERP"
        ) {
            ...
            .then((day_end_result) => {
              ...
              if (day_end_data.posted === "N") {
                _mysql
                  ...
                  INSERT INTO hims_f_salary_history (salary_header_id, salary_number,month,year,employee_id,salary_date,salary_type,net_salary,hospital_id, created_by, created_date, revert_reason) 
              VALUE(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ;`,
                    values: [
