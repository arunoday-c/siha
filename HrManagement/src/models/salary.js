...
export default {
  ...
  getSalaryProcess: (req, res, next) => {
    try {
      ...
      let _stringData =
        inputParam.employee_id != null ? ` and S.employee_id= ${inputParam.employee_id}` : "";
      ...
      _stringData +=
        inputParam.department_id != null ? ` and SD.department_id= ${inputParam.department_id}` : "";

      _stringData +=
        inputParam.group_id != null ? ` and emp.employee_group_id= ${inputParam.group_id}` : "";
      ...
      _mysql
        .executeQuery({
          query:
            ...
            _stringData,
          values: [inputParam.year, inputParam.month, inputParam.year, inputParam.month, inputParam.hospital_id],
          printQuery: true
        })
        .then(salary_process => {
          if (salary_process.length > 0) {
            let _salary_data = _.filter(salary_process, f => {
              return f.from_normal_salary === 'Y' || f.hims_f_employee_annual_leave_id === null;
            });
            if (inputParam.employee_id != null) {
              let _annual_salary = _.filter(salary_process, f => {
                return f.from_normal_salary === 'N' || f.hims_f_employee_annual_leave_id !== null;
              });
              if (_annual_salary.length > 0) {
                req.records = {
                  invalid_input: true,
                  message: "Selected Employee applied annual leave, Please process from annual leave salary screen. "
                };
                next();
                return;
              }
