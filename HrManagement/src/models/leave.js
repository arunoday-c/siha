      ...
      _mysql
        ...
        .then((branch) => {
          ...
          _mysql
            ...
            weekoff_days,holidays,leave_from,absent_id,is_projected_leave,is_across_year_leave,employee_joined,leave_category,from_year_applied_days,to_year_applied_days, created_date, created_by, updated_date, updated_by)\
            VALUE(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
              values: [
                  ...
                  if (
                    ...
                    if (isHoliday.length > 0) {
                      ...
                      if (
                        ...
                        // console.log("11", input.cancel)
                        // console.log("12", currentClosingBal)
                        // console.log("13", calculatedLeaveDays)
                        // console.log("14", annual_leave)
                        // console.log("15", input.is_across_year_leave)
                        // console.log("16", moment(input.to_date, "YYYY-MM-DD").format(
                        //   "YYYYMMDD"
                        // ))
                        // console.log("17", moment(input.to_date, "YYYY-MM-DD").format("YYYYMMDD"))
                        // if (
                        //   (currentClosingBal < calculatedLeaveDays &&
                        //     annual_leave == "Y" &&
                        //     input.cancel != "Y") ||
      ...
      try {
        ...
        if (not_clashing_sessions.length > 0) {
          ...
          req.records = {
            ...
            message:
              "Leave application exists between (" +
