...
//ST------calculate begning_of_leave and end_of_leave and leaveDays in leaveDates Range
    ...
    if(allLeaves.length>0){
      ...
      .Where(w =>    
        (((w.holiday_date ==from_date && w.weekoff=='Y')||  (w.holiday_date ==from_date&& w.holiday=='Y' && w.holiday_type == "RE") ||
        (w.holiday_date ==from_date&& w.holiday=='Y' && w.holiday_type == "RS" &&w.religion_id==my_religion)         
        )|| ((w.holiday_date ==to_date && w.weekoff=='Y')||  (w.holiday_date ==to_date&& w.holiday=='Y' && w.holiday_type == "RE") ||
        (w.holiday_date ==to_date&& w.holiday=='Y' && w.holiday_type == "RS" &&w.religion_id==my_religion) )))
      .Select(s => {
        return {         
          holiday_date: s.holiday_date,
          holiday_description: s.holiday_description
...
// -------END OF--- get count of holidays and weekOffs betwen apllied leave range
      ...
      if(isHoliday.length>0){
        ...
        req.records = {
          invalid_input: true,
          message: `you cant apply leave, ${isHoliday[0].holiday_date} is : ${isHoliday[0].holiday_description}`
        };
        next();
        return;
      }else{
  ...
  for (let k = 0; k < dateRange.length; k++) {
    if (input.from_session == "SH" && k == 0) {
      if (from_month === to_month && input.to_session == "FH") {
        ...
        leaveDeductionArray.push({
          month_name: dateRange[k]["month_name"],
          finalLeave: parseFloat(dateRange[k]["leaveDays"]) - parseFloat(0.5)
        });
      }
    } else if (
      input.to_session == "FH" &&
