        ...
        _mysql
          .executeQuery({
            query:
              ...
              hims_f_daily_time_sheet TS, hims_d_employee E, hims_f_project_roster PR where \
              TS.employee_id=E.hims_d_employee_id and PR.employee_id = TS.employee_id and date(TS.attendance_date)=date(PR.attendance_date) and \
              E.hospital_id=? and PR.project_id=? " +
              strQuery,
            values: inputValue,
            printQuery: true
          })
...
.executeQuery({
  query:
    ` select hims_f_daily_time_sheet_id,TS.employee_id,TS.attendance_date,in_time,out_time,worked_hours,\
...
.then(time_sheet => {
        ...
        if (time_sheet.length > 0) {
          ...
          .executeQuery({
            ...
            to_date from hims_f_leave_application inner join hims_d_leave L on LA.leave_id=L.hims_d_leave_id \
          ...
          .then(result => {
            ...
            if(All_Project_Roster.length>0 &&input.select_wise == "M"&&input.employee_id >0){
              ...
              for (let i = 0; i < date_range.length; i++) {
                  ...
                  .Where(w=>w.attendance_date==moment(date_range[i]).format("YYYY-MM-DD")).Select(s=>{
                    return{
              ...
              if(leave!=null ){
                outputArray.push(leave);
                 ...
                 if(holiday_or_weekOff.weekoff=="Y"){
                  outputArray.push({ attendance_date:moment(date_range[i]).format("YYYY-MM-DD"),status:"WO" });

                 }else if(holiday_or_weekOff.holiday=="Y"){
                  outputArray.push({attendance_date: moment(date_range[i]).format("YYYY-MM-DD"),status:"HO"  });
                 }
               
              }else{
                outputArray.push({ attendance_date:moment(date_range[i]).format("YYYY-MM-DD") ,
                status:"AB"});
              }
