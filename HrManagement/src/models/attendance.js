          ...
          .then(result => {
            ...
            if ( AllEmployees.length > 0 &&
             ...
             biometric_ids = new LINQ(AllEmployees)   
              .Select(s =>s. biometric_id).ToArray();
                  ...
                  function(err, attResult) {
                    ...
                    if (attendcResult.length > 0 && from_date==to_date) {
                      ...
                      for(let i=0;i<AllEmployees.length;i++){
                         ...
                         biometricData.push(new LINQ(attendcResult).Where(w=>w.UserID==AllEmployees[i]["biometric_id"])
                        .Select(s =>{
                          return{
                            biometric_id:s.UserID,
                            attendance_date:moment(s.Date, "MM-DD-YYYY").format("YYYY-MM-DD"),
...
function insertTimeSheet(returnQry,
  biometricData,
  AllLeaves,
  allHolidays,
  from_date,
  ...
  try {
    ...
    _mysql
      .executeQueryWithTransaction({
        query: "INSERT INTO hims_f_daily_time_sheet(??) VALUES ?  ON DUPLICATE KEY UPDATE employee_id=values(employee_id),\
