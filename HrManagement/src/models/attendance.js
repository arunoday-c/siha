...
module.exports = {
		...
		_mysql
			...
			.then((result) => {
				if (input.absent_id > 0) {
					_mysql
						.executeQuery({
							query: "update hims_f_absent  set status='CPR' where hims_f_absent_id=?;",
											...
											for (let i = 0; i < allEmployees.length; i++) {
												...
												if (
													...
													allEmployees[i]['defaults'].total_work_days -= moment(
														endOfMonth,
														'YYYY-MM-DD'
													).diff(moment(allEmployees[i]['exit_date'], 'YYYY-MM-DD'), 'days');
												} else if (
													allEmployees[i]['date_of_joining'] > startOfMonth &&
													allEmployees[i]['exit_date'] < endOfMonth
												) {
													allEmployees[i]['defaults'].total_work_days -=
														moment(allEmployees[i]['date_of_joining'], 'YYYY-MM-DD').diff(
												...
												allEmployees[i]['defaults'].emp_absent_days = new LINQ(allAbsents)
													.Where(
												...
												allEmployees[i]['defaults'].paid_leave = new LINQ(allMonthlyLeaves)
													.Where(
														(w) =>
												...
												allEmployees[i]['defaults'].unpaid_leave = new LINQ(allMonthlyLeaves)
													.Where(
														(w) =>
															w.employee_id == allEmployees[i]['hims_d_employee_id'] &&
															w.leave_type == 'U'
												...
												allEmployees[i]['defaults'].holiday_include = new LINQ(allMonthlyLeaves)
													.Where(
														(w) =>
															w.employee_id == allEmployees[i]['hims_d_employee_id'] &&
															w.include_holiday == 'Y'
												...
												allEmployees[i]['defaults'].pending_leaves = new LINQ(allPendingLeaves)
													.Where(
														(w) => w.employee_id == allEmployees[i]['hims_d_employee_id']
													)
												...
												if (
													...
													allEmployees[i]['defaults'].total_holidays = new LINQ(allHolidays)
														.Where(
															(w) =>
													...
													allEmployees[i][
														'defaults'
													].total_week_off = _.filter(allHolidays, (obj) => {
														return (
															obj.weekoff === 'Y' &&
															obj.holiday_type === 'RE' &&
												...
												attendanceArray.push({
													...
													total_weekoff_days: allEmployees[i]['defaults'].total_week_off,
							...
							.then((results) => {
								...
								for (let i = 0; i < attResult.length; i++) {
									...
									insertArray.push({
										...
										total_leave: attResult[i]['paid_leave'] + attResult[i]['unpaid_leave'],
	...
	postTimeSheet: (req, res, next) => {
		...
		let dailyAttendance = [];
		let fetchData = '';
		if (input.hospital_id > 0 && (input.hims_d_employee_id > 0 || input.sub_department_id > 0)) {
			if (input.hims_d_employee_id > 0) {
						...
						if (excptions.length > 0) {
							...
							for (let i = 0; i < result.length; i++) {
								...
								if (result[i]['status'] == 'PR') {
									...
									let worked_minutes =
										parseInt(result[i]['hours'] * 60) + parseInt(result[i]['minutes']);
								...
								dailyAttendance.push({
									...
									working_hours: result[i]['actual_hours'] + '.' + result[i]['actual_minutes'],
										...
										.then((attResult) => {
											...
											for (let i = 0; i < attResult.length; i++) {
												insertArray.push({
													...
													total_working_hours: attResult[i]['total_working_hours'],
													shortage_hours:
														attResult[i]['shortage_hourss'] - attResult[i]['ot_hourss'] >= 0
															? attResult[i]['shortage_hourss'] -
																attResult[i]['ot_hourss']
															: 0,
													ot_work_hours:
														attResult[i]['ot_hourss'] - attResult[i]['shortage_hourss'] >= 0
															? attResult[i]['ot_hourss'] -
																attResult[i]['shortage_hourss']
															: 0
												});
				...
				_mysql
					...
					.then((options) => {
						if (
							...
							cut_off_date =
								moment(input.to_date).clone().format('YYYY-MM-') + options[0]['payroll_payment_date'];
							...
							const prevMonthYear = moment(input.from_date).clone().add(-1, 'months');
										...
										if (excptions.length > 0) {
												...
												RosterAttendance.push(
													new LINQ(RosterResult)
														...
														.FirstOrDefault({
															...
															weekoff_days:
																whichLeave == 0 && holiday_or_weekOff.weekoff == 'Y'
																	? 1
																	: 0,
															holidays:
																whichLeave == 0 && holiday_or_weekOff.holiday == 'Y'
																	? 1
																	: 0,
															present_days:
																whichLeave == 0 &&
											...
											for (let i = 0; i < LastTenDaysResult.length; i++) {
												...
												previousMonthData.push({
													...
													sub_department_id: LastTenDaysResult[i]['sub_department_id'],
													attendance_date: LastTenDaysResult[i]['attendance_date'],
													year: moment(LastTenDaysResult[i]['attendance_date']).format(
														'YYYY'
													),
													month: moment(LastTenDaysResult[i]['attendance_date']).format('M'),
													total_days: 1,
													present_days: LastTenDaysResult[i]['status'] == 'PR' ? 1 : 0,
													absent_days: LastTenDaysResult[i]['status'] == 'AB' ? 1 : 0,
													total_work_days: LastTenDaysResult[i]['status'] == 'PR' ? 1 : 0,
													weekoff_days: LastTenDaysResult[i]['status'] == 'WO' ? 1 : 0,
													holidays: LastTenDaysResult[i]['status'] == 'HO' ? 1 : 0,
													paid_leave: LastTenDaysResult[i]['status'] == 'PL' ? 1 : 0,
													unpaid_leave: LastTenDaysResult[i]['status'] == 'UL' ? 1 : 0,
													...
													shortage_minutes:
														LastTenDaysResult[i]['consider_ot_shrtg'] == 'Y'
															...
															: 0,
													ot_work_hours:
														LastTenDaysResult[i]['consider_ot_shrtg'] == 'Y' ? ot_time : 0,
													ot_minutes:
														LastTenDaysResult[i]['consider_ot_shrtg'] == 'Y' ? ot_min : 0
												});
											}
					...
					if (results.affectedRows > 0) {
						...
						_mysql
							...
							.then((result) => {
								...
								let total_min = parseInt(sum_actual_hour * 60) + parseInt(sum_actual_min);
								...
								let worked_min = parseInt(sum_work_hour * 60) + parseInt(sum_work_min);
				...
				for (let i = 0; i < result.length; i++) {
					outputArray.push({
						...
						complete_shortage_hr:
							...
							'.' +
							String('0' + parseInt(result[i]['shortage_minutes'])).slice(-2),
						...
						complete_weekoff_ot_hr:
							...
							'.' +
							String('0' + parseInt(result[i]['ot_weekoff_minutes'])).slice(-2),
						complete_holiday_ot_hr:
							result[i]['ot_holiday_hours'] +
							'.' +
							String('0' + parseInt(result[i]['ot_holiday_minutes'])).slice(-2)
					});
				}
									...
									if (AttenResult.length > 0) {
										for (let i = 0; i < AttenResult.length; i++) {
											...
											if (AttenResult[i]['status'] == 'WO') {
												...
												//calculating over time
												week_off_ot_hour = parseInt(parseInt(Math.abs(worked_minutes)) / parseInt(60));
											...
											if (AttenResult[i]['status'] == 'HO') {
												...
												//calculating over time
												holiday_ot_hour = parseInt(parseInt(Math.abs(worked_minutes)) / parseInt(60));
											...
											dailyAttendance.push({
												...
												total_hours:
													AttenResult[i]['consider_ot_shrtg'] == 'Y'
														? AttenResult[i]['worked_hours']
														: AttenResult[i]['actual_hours'] + '.' + AttenResult[i]['actual_minutes'],
												hours:
													AttenResult[i]['consider_ot_shrtg'] == 'Y'
														? AttenResult[i]['hours']
														: AttenResult[i]['actual_hours'],
												minutes:
													AttenResult[i]['consider_ot_shrtg'] == 'Y'
														? AttenResult[i]['minutes']
														: AttenResult[i]['actual_minutes'],
												working_hours: AttenResult[i]['actual_hours'] + '.' + AttenResult[i]['actual_minutes'],
		...
		if (singleEmployee == 'N') {
			for (let i = 0; i < biometricData.length; i++) {
				if (
					...
					(biometricData[i]['in_time'] != null && biometricData[i]['out_time'] == null) ||
					(biometricData[i]['in_time'] == null && biometricData[i]['out_time'] != null)
				) {
					//Exception
			...
			.then((finalResult) => {
				...
				_mysql
					...
					.then((result) => {
						...
						for (let i = 0; i < result.length; i++) {
							let total_minutes =
								parseInt(result[i]['actual_hours'] * 60) + parseInt(result[i]['actual_minutes']);
							let worked_minutes = parseInt(result[i]['hours'] * 60) + parseInt(result[i]['minutes']);
