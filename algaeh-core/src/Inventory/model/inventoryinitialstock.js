...
const { updateIntoInvItemLocation } = commonFunction;
...
let addInventoryInitialStock = (req, res, next) => {
  try {
    if (req.db == null) {
          ...
          connection.query(
            ...
            (error, headerResult) => {
              ...
              if (headerResult.insertId != null) {
                ...
                connection.query(
                  "INSERT INTO hims_f_inventory_stock_detail(" +
                    insurtColumns.join(",") +
                    ",inventory_stock_header_id,created_date,updated_date) VALUES ?",
                  [
                    jsonArrayToObject({
...
let getInventoryInitialStock = (req, res, next) => {
  let selectWhere = {
    document_number: "ALL"
  };
  try {
        ...
        (error, headerResult) => {
          ...
          if (headerResult.length != 0) {
            debugLog(
              ...
              headerResult[0].hims_f_inventory_stock_header_id
            );
            connection.query(
...
let updateInventoryInitialStock = (req, res, next) => {
  let initialStock = {
    posted: null,
