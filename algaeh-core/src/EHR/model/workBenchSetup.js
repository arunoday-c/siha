    ...
    let $max = _mysql
      ...
      .then((result) => {
        ...
        _mysql
          .executeQuery({
            query:
              "INSERT INTO `hims_d_vitals_header` (vitals_name,vital_short_name,sequence_order, uom,general,display,isDecimal,record_status,created_date, created_by, updated_date, updated_by,hims_d_vitals_header_id)\
      VALUE(?,?,?,?,?,?,?,?,?,?,?,?,?)",
            values: [
              input.vitals_name,
...
let getVitalMasterHeader = (req, res, next) => {
  ...
  try {
    ...
    _mysql
      .executeQuery({
        query:
          "select hims_d_vitals_header_id,uom, vitals_name,vital_short_name,LAST_INSERT_ID(sequence_order) as last_inserted,sequence_order,general,display,isDecimal,record_status,box_type FROM hims_d_vitals_header where " +
          where.condition +
          " order by hims_d_vitals_header_id desc",
        values: [...where.values],
        printQuery: true,
...
let updateVitalMasterHeader = (req, res, next) => {
  try {
    ...
    _mysql
      .executeQuery({
        query:
          "UPDATE `hims_d_vitals_header` SET vitals_name=?,vital_short_name=?,sequence_order=?,uom=?,general=?,display=? ,isDecimal=?,record_status=?,\
