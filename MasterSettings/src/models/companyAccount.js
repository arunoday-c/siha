  ...
  getCompanyAccount: (req, res, next) => {
    ...
    try {
      _mysql
        .executeQuery({
          query:
            "select  hims_d_company_account_id,employer_cr_no,payer_cr_no,bank_id,bank_short_name,account_number from\
            hims_d_company_account where record_status='A' order by hims_d_company_account_id desc "
        })
        .then(result => {
          _mysql.releaseConnection();
          req.records = result;
