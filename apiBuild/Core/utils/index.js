...
var releaseConnection = function releaseConnection(req, res) {
  if (req.db != null) {
    delete req.db;
  }
  if (req.records != null) {
 ...
 `postfix` >= `postfix_start` AND `postfix` <= `postfix_end`", [options.module_desc], function (error, result) {
    ...
    if (result.length == 0) {
      ...
      result.map(function (item, index) {
        ...
        var length = parseInt(item["length"], 10) - parseInt(prefix.length, 10);
        var increment_by = parseInt(item["increment_by"], 10);
        if (options.counter != null) {
          increment_by = increment_by + parseInt(options.counter - 1, 10);
        }

        var numgen_seperator = item["numgen_seperator"];
        var newNumber = parseInt(postfix, 10) + increment_by;
...
var runningNumber = function runningNumber(db, numgenId, paramName, callBack, isreleaseConnection) {
  ...
  ,`postfix_end`, `current_num`, `pervious_num` FROM `hims_f_app_numgen`\
   WHERE record_status='A' AND hims_f_app_numgen_id=?", [numgenId], function (error, result) {
    ...
    var length = parseInt(result["length"], 10) - parseInt(prefix.length, 10);
    ...
    var newNumber = parseInt(postfix, 10) + parseInt(increment_by, 10);

    if (parseInt(postfix_start, 10) <= newNumber && parseInt(postfix_end, 10) >= newNumber) {
...
var bulkMasters = function bulkMasters(fileName, bulkObject) {
  try {
    var testobj = JSON.stringify(bulkObject);
...
var generateDbConnection = function generateDbConnection(req, res, next) {
  var _db = req.db;
  if (_db == null) {
  ...
  _db.getConnection(function (error, connection) {
    ...
    (0, _logging.debugLog)("connection", connection);
    next();
  });
};

module.exports = {
  generateDbConnection: generateDbConnection,
