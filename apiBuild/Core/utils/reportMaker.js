...
var algaehReportConfig = function algaehReportConfig(reportName) {
  var queries = {
    allQueries: [{
      reportName: "patientData",
      reportQuery: "select hims_f_patient_appointment_id, patient_id, patient_code, provider_id, sub_department_id, number_of_slot,\
  ...
  var row = new _nodeLinq.LINQ(queries.allQueries).Where(function (w) {
    return String(w.reportName).toUpperCase() === String(reportName).toUpperCase();
  }).FirstOrDefault();
  return row;
};

module.exports = {
  algaehReportConfig: algaehReportConfig
};
