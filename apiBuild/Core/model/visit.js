...
var insertPatientVisitData = function insertPatientVisitData(req, res, next) {
  try {
    ...
    var internalInsertPatientVisitData = function internalInsertPatientVisitData() {
      ...
      (0, _logging.debugFunction)("2");

      (0, _logging.debugLog)("internal Inside: ", existingExparyDate);
      (0, _logging.debugLog)("internal Inside: ", currentPatientEpisodeNo);
      (0, _logging.debugLog)("today: ", today);
      if ((existingExparyDate != null || existingExparyDate != undefined) && (0, _moment2.default)(existingExparyDate).format("YYYY-MM-DD") >= today) {
        (0, _logging.debugFunction)("Inside");
        inputParam.visit_expiery_date = existingExparyDate;
        inputParam.episode_id = currentPatientEpisodeNo;
      }
      (0, _logging.debugFunction)("3");
...
VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?, ?, ?);", [inputParam.patient_id, inputParam.visit_type, inputParam.age_in_years, inputParam.age_in_months, inputParam.age_in_days, inputParam.insured, inputParam.sec_insured, new Date(), inputParam.department_id, inputParam.sub_department_id, inputParam.doctor_id, inputParam.maternity_patient, inputParam.is_mlc, inputParam.mlc_accident_reg_no, inputParam.mlc_police_station, inputParam.mlc_wound_certified_date != null ? new Date(inputParam.mlc_wound_certified_date) : inputParam.mlc_wound_certified_date, inputParam.existing_plan, inputParam.treatment_plan_id, inputParam.created_by, new Date(), inputParam.visit_code, inputParam.visit_expiery_date != null ? new Date(inputParam.visit_expiery_date) : inputParam.visit_expiery_date, inputParam.episode_id, inputParam.appointment_id, inputParam.appointment_patient], function (error, visitresult) {
        if (error) {
...
, `patient_message`, `is_critical_message`, `message_active_till`, `created_by`, `created_date`\
    ...
    if (inputParam.consultation == "Y") {
      ...
      db.query(" select max(visit_expiery_date) as visit_expiery_date,max(episode_id) as episode_id from hims_f_patient_visit where\
         patient_id=? and doctor_id=? and record_status='A';", [inputParam.patient_id, inputParam.doctor_id], function (error, expResult) {
        ...
        if (error) {
          ...
          (0, _logging.debugLog)("Else ", expResult);
          if (inputParam.existing_plan === "Y") {
            inputParam.visit_expiery_date = (0, _moment2.default)(expResult[0]["visit_expiery_date"]).format("YYYY-MM-DD");
            ...
            if (expResult[0].visit_expiery_date != null && expResult[0].episode_id != null) {
              ...
              (0, _logging.debugLog)("expResult Inside ", existingExparyDate);
              (0, _logging.debugLog)("expResult Inside ", currentPatientEpisodeNo);
            }
            ...
            if (existingExparyDate == null || existingExparyDate == undefined || existingExparyDate < today) {
              ...
              db.query("SELECT param_value,episode_id from algaeh_d_app_config WHERE algaeh_d_app_config_id=11 \
                ...
                if (error) {
                  ...
                  inputParam.visit_expiery_date = (0, _moment2.default)().add(parseInt(record[0]["param_value"], 10), "days").format("YYYY-MM-DD");
      ...
      dataBase.query(" select max(visit_expiery_date) as visit_expiery_date,episode_id from hims_f_patient_visit where\
        ...
        if (existingExparyDate == null || existingExparyDate == undefined || existingExparyDate < today) {
          ...
          dataBase.query("SELECT param_value from algaeh_d_app_config WHERE param_name=?", ["VISITEXPERIDAY"], function (error, record) {
            ...
            inputParam.visit_expiery_date = (0, _moment2.default)().add(record != null && record.length != 0 ? parseInt(record[0]["param_value"], 10) : 0, "days").format("YYYY-MM-DD");
...
var closeVisit = function closeVisit(req, res, next) {
  try {
    if (req.db == null) {
      next(_httpStatus2.default.dataBaseNotInitilizedError());
    }
    ...
    db.getConnection(function (error, connection) {
      ...
      for (var i = 0; i < inputParam.length; i++) {
        qry += " UPDATE `hims_f_patient_visit` SET visit_status='" + inputParam[i].visit_status + "' WHERE hims_f_patient_visit_id='" + details[i].hims_f_patient_visit_id + "';";
      }
