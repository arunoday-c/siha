...
var getPatientMrdList = function getPatientMrdList(req, res, next) {
  var selectWhere = {
    patient_code: "ALL",
  ...
  try {
    ...
    if (req.query.registration_date != "null" && req.query.registration_date != null) {
    ...
    db.getConnection(function (error, connection) {
      connection.query("select hims_d_patient_id,patient_code,registration_date,first_name,middle_name,\
        ...
        P.nationality_id=N.hims_d_nationality_id and P.primary_identity_id=DOC.hims_d_identity_document_id  " + patientName + "" + registration_date + " AND " + where.condition, where.values, function (error, result) {
...
var getPatientEncounterDetails = function getPatientEncounterDetails(req, res, next) {
  try {
    if (req.db == null) {
      next(_httpStatus2.default.dataBaseNotInitilizedError());
    }
    ...
    db.getConnection(function (error, connection) {
      connection.query("select hims_f_patient_encounter_id, PE.patient_id,P.full_name,PE.provider_id,E.full_name as provider_name, visit_id,\
        V.insured,V.sec_insured,V.sub_department_id,SD.sub_department_name,PE.episode_id,PE.encounter_id,PE.updated_date as encountered_date,\
         ...
         IM.secondary_insurance_provider_id=IPR.hims_d_insurance_provider_id   where PE.record_status='A' and P.record_status='A' and E.record_status='A' \
                 ...
                 order by encountered_date desc;", [req.query.patient_id], function (error, result) {
        (0, _utils.releaseDBConnection)(db, connection);
        if (error) {
          next(error);
        }
...
var getPatientChiefComplaint = function getPatientChiefComplaint(req, res, next) {
  try {
    if (req.db == null) {
      next(_httpStatus2.default.dataBaseNotInitilizedError());
    }
    ...
    db.getConnection(function (error, connection) {
      connection.query("select hims_f_episode_chief_complaint_id ,episode_id,chief_complaint_id,HH.hpi_description as chief_complaint\
        from hims_f_episode_chief_complaint ECC,hims_d_hpi_header HH\
        Where ECC.record_status='A' and HH.record_status='A' \
        and ECC.chief_complaint_id=HH.hims_d_hpi_header_id and episode_id=?", [req.query.episode_id], function (error, result) {
        (0, _utils.releaseDBConnection)(db, connection);
        if (error) {
          next(error);
        }
...
var getPatientDiagnosis = function getPatientDiagnosis(req, res, next) {
  ...
  try {
    ...
    db.getConnection(function (error, connection) {
      connection.query("select hims_f_patient_diagnosis_id, patient_id, episode_id, daignosis_id,ICD.icd_code as daignosis_code,\
        ...
        PD.created_date as diagnosis_date  from hims_f_patient_diagnosis PD,hims_d_icd ICD\
         ...
         and PD.daignosis_id=ICD.hims_d_icd_id and " + where.condition + " order by diagnosis_date desc", where.values, function (error, result) {
        (0, _utils.releaseDBConnection)(db, connection);
        if (error) {
          next(error);
        }
...
var getPatientMedication = function getPatientMedication(req, res, next) {
  var selectWhere = {
    patient_id: "ALL",
    encounter_id: "ALL"
  };
  try {
    ...
    db.getConnection(function (error, connection) {
      connection.query("select  hims_f_prescription_id, patient_id, encounter_id, provider_id, episode_id,\
        ...
        and PD.generic_id =IG.hims_d_item_generic_id and " + where.condition + " order by prescription_date desc", where.values, function (error, result) {
        (0, _utils.releaseDBConnection)(db, connection);
        if (error) {
          next(error);
        }
...
var getPatientInvestigation = function getPatientInvestigation(req, res, next) {
  try {
    if (req.db == null) {
      next(_httpStatus2.default.dataBaseNotInitilizedError());
    }
    ...
    db.getConnection(function (error, connection) {
      connection.query("select hims_f_ordered_services_id, OS.patient_id, OS.visit_id,visit_date, OS.doctor_id,\
        ...
        from  hims_f_ordered_services OS \
         ...
         where OS." + conString + " group by hims_f_ordered_services_id order by OS.visit_id desc", function (error, result) {
        (0, _utils.releaseDBConnection)(db, connection);
        if (error) {
          next(error);
        }
...
var getPatientPaymentDetails = function getPatientPaymentDetails(req, res, next) {
  try {
    if (req.db == null) {
      next(_httpStatus2.default.dataBaseNotInitilizedError());
    }
    ...
    db.getConnection(function (error, connection) {
      connection.query("select distinct  visit_id from hims_f_billing_header where record_status='A' and patient_id=? order by visit_id desc;", [req.query.patient_id], function (error, result) {
        if (error) {
          (0, _utils.releaseDBConnection)(db, connection);
          next(error);
        }

        var allVisits = new _nodeLinq.LINQ(result).Where(function (w) {
          return w.visit_id != null;
        }).Select(function (s) {
          return s.visit_id;
        }).ToArray();

        (0, _logging.debugLog)("allVisits:", allVisits);

        var outputArray = [];
        if (result.length > 0) {
          var _loop = function _loop(i) {
            connection.query("select hims_f_billing_header_id ,bill_number,patient_id,visit_id,E.full_name provider_name,incharge_or_provider,bill_date,\
              net_amount,patient_payable,receiveable_amount,credit_amount from hims_f_billing_header BH,hims_d_employee E where BH.record_status='A' and\
               E.record_status='A' and BH.incharge_or_provider=E.hims_d_employee_id and visit_id=? order by bill_date desc;", [allVisits[i]], function (error, billHeadResult) {
              if (error) {
                (0, _utils.releaseDBConnection)(db, connection);
                next(error);
              }
              (0, _logging.debugLog)("inside billHeadResult:", billHeadResult);
              (0, _logging.debugLog)("zzzzz:", outputArray);
              if (billHeadResult.length > 0) {
                var _loop2 = function _loop2(k) {
                  new Promise(function (resolve, reject) {
                    try {
                      if (billHeadResult.length == 0) {
                        (0, _logging.debugLog)("blank :");
                        return resolve(billHeadResult);
                      } else {
                        connection.query("select hims_f_receipt_header_id, receipt_number, receipt_date, billing_header_id, total_amount\
                          from hims_f_receipt_header where record_status='A' and billing_header_id=?;", [billHeadResult[k].hims_f_billing_header_id], function (error, recptResult) {
                          if (error) {
                            (0, _utils.releaseDBConnection)(db, connection);
                            next(error);
                          }
                  ...
                  }).then(function (resultRCPT) {
                    connection.query("select BH.hims_f_billing_header_id,company_payable as pri_company_payble, sec_company_payable,\
                          ...
                          where BH.record_status='A'  and   BH.hims_f_billing_header_id=?", [billHeadResult[k].hims_f_billing_header_id], function (error, insResult) {
                      if (error) {
                        (0, _utils.releaseDBConnection)(db, connection);
                        next(error);
                      }
                      (0, _logging.debugLog)("inside insurance:", insResult);
                      outputArray.push(_extends({}, billHeadResult[k], {
                        prov_date: (0, _moment2.default)(billHeadResult[k].bill_date).format("DD-MM-YYYY") + " " + billHeadResult[k].provider_name
                      }, insResult[0], {
                        receipt: resultRCPT
                      }));
                ...
                for (var k = 0; k < billHeadResult.length; k++) {
                  _loop2(k);
                }
              } else {
                if (i == allVisits.length - 1) {
                  (0, _logging.debugLog)("inside else no bill head:");
                  (0, _utils.releaseDBConnection)(db, connection);
                  req.records = outputArray;

                  (0, _logging.debugLog)("eee:", outputArray);
                  next();
                }
              }
            });
...
var getPatientTreatments = function getPatientTreatments(req, res, next) {
  try {
    if (req.db == null) {
      next(_httpStatus2.default.dataBaseNotInitilizedError());
    }
    ...
    db.getConnection(function (error, connection) {
      connection.query("select hims_d_service_type_id from hims_d_service_type  where \
        record_status='A' and service_type='Procedure'", function (error, result) {
        if (error) {
          (0, _utils.releaseDBConnection)(db, connection);
          next(error);
        }
        ...
        if (result.length > 0) {
          connection.query("select hims_f_ordered_services_id,OS.patient_id,OS.doctor_id,E.full_name as doctor_name,OS.service_type_id,V.visit_date,services_id,\
        ...
        and OS.service_type_id=? and OS.patient_id=? order by visit_date desc", [result[0].hims_d_service_type_id, req.query.patient_id], function (error, procResult) {
            (0, _utils.releaseDBConnection)(db, connection);
            if (error) {
              next(error);
            }
...
module.exports = {
  getPatientMrdList: getPatientMrdList,
  getPatientEncounterDetails: getPatientEncounterDetails,
  getPatientChiefComplaint: getPatientChiefComplaint,
  getPatientDiagnosis: getPatientDiagnosis,
  getPatientMedication: getPatientMedication,
  getPatientInvestigation: getPatientInvestigation,
  getPatientPaymentDetails: getPatientPaymentDetails,
