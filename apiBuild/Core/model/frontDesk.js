...
var addFrontDeskBACKUp = function addFrontDeskBACKUp(req, res, next) {
                ...
                return new _bluebird2.default(function (resolve, reject) {
                  ...
                  return new _bluebird2.default(function (resolve, reject) {
                    ...
                    return new _bluebird2.default(function (resolve, reject) {
                      ...
                      (0, _logging.debugLog)("inside front desk chier functionalityL:");
                      (0, _billing.addCashHandover)(req, res, next);
                    }).then(function (cashier_result) {
                      connection.commit(function (error) {
                        (0, _logging.debugLog)("After Episode Error: ", error);
                        if (error) {
                          (0, _logging.debugLog)("Error: ", error);
                        ...
                        (0, _logging.debugLog)("encounterResult: ", cashier_result);
                        if (requestCounter != 0) requestCounter = requestCounter - 1;
                        (0, _utils.releaseDBConnection)(db, connection);
                        next();
                      });
...
var updateFrontDeskBackup = function updateFrontDeskBackup(req, res, next) {
...
var getCashHandoverDetails = function getCashHandoverDetails(req, res, next) {
  // let selectWhere = {
  //   column: "ALL"
  // };
  try {
    ...
    if (req.query.shift_status != "null" && req.query.shift_status != null && req.query.shift_status != undefined) {
      shift_status = "and shift_status='" + req.query.shift_status + "'";
    }

    db.getConnection(function (error, connection) {
      connection.query("select hims_f_cash_handover_header_id, shift_id, daily_handover_date,\
       ...
       cheque_status, remarks, no_of_cheques,EDM.user_id,E.full_name as employee_name,E.arabic_name as employee_arabic_name \
        ...
        date(daily_handover_date)=date(?) " + shift_status, [req.query.shift_id, req.query.daily_handover_date], function (error, result) {
        (0, _utils.releaseDBConnection)(db, connection);
        if (error) {
          next(error);
        }
...
var updateCashHandoverDetails = function updateCashHandoverDetails(req, res, next) {
  try {
    if (req.db == null) {
      next(_httpStatus2.default.dataBaseNotInitilizedError());
    }
    ...
    db.getConnection(function (error, connection) {
      ...
      connection.query("UPDATE `hims_f_cash_handover_detail` SET  shift_status=?,close_date=?,close_by=?,actual_cash=?,\
        difference_cash=?,cash_status=?,actual_card=?,difference_card=?,card_status=?,actual_cheque=?,\
        difference_cheque=?,cheque_status=?,remarks=?,\
           updated_date=?, updated_by=?  WHERE  `record_status`='A' and `hims_f_cash_handover_detail_id`=?;", [input.shift_status, input.close_date, input.close_by, input.actual_cash, input.difference_cash, input.cash_status, input.actual_card, input.difference_card, input.card_status, input.actual_cheque, input.difference_cheque, input.cheque_status, input.remarks, new Date(), input.updated_by, input.hims_f_cash_handover_detail_id], function (error, result) {
        (0, _utils.releaseDBConnection)(db, connection);
        if (error) {
          next(error);
        }
...
module.exports = {
  ...
  getCashHandoverDetails: getCashHandoverDetails,
  updateCashHandoverDetails: updateCashHandoverDetails
};
