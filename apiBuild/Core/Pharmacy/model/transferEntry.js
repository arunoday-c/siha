    ...
    db.getConnection(function (error, connection) {
      ...
      connection.beginTransaction(function (error) {
        ...
        return new _bluebird2.default(function (resolve, reject) {
          ...
          connection.query("INSERT INTO `hims_f_pharmacy_transfer_header` (transfer_number,transfer_date,`year`,period,\
            ...
            if (headerResult.insertId != null) {
              ...
              connection.query("INSERT INTO hims_f_pharmacy_transfer_detail(" + insurtColumns.join(",") + ",transfer_header_id) VALUES ?", [(0, _utils.jsonArrayToObject)({
...
var gettransferEntry = function gettransferEntry(req, res, next) {
  ...
  try {
    ...
    db.getConnection(function (error, connection) {
      connection.query("SELECT * from  hims_f_pharmacy_transfer_header\
        ...
        if (headerResult.length != 0) {
          (0, _logging.debugLog)("hims_f_pharmacy_transfer_header_id: ", headerResult[0].hims_f_pharmacy_transfer_header_id);
          connection.query("select * from hims_f_pharmacy_transfer_detail where transfer_header_id=?", headerResult[0].hims_f_pharmacy_transfer_header_id, function (error, pharmacy_stock_detail) {
            if (error) {
              (0, _utils.releaseDBConnection)(db, connection);
              next(error);
            }
  ...
  try {
    ...
    connection.beginTransaction(function (error) {
      ...
      return new _bluebird2.default(function (resolve, reject) {
        ...
        connection.query("UPDATE `hims_f_pharmacy_transfer_header` SET `completed`=?, `completed_date`=? \
          WHERE `hims_f_pharmacy_transfer_header_id`=?", [inputParam.completed, new Date(), inputParam.hims_f_pharmacy_transfer_header_id], function (error, result) {
        ...
        return new _bluebird2.default(function (resolve, reject) {
          ...
          //Update To location
          for (var i = 0; i < req.body.pharmacy_stock_detail.length; i++) {
            req.body.pharmacy_stock_detail[i].location_id = req.body.to_location_id;
            req.body.pharmacy_stock_detail[i].location_type = req.body.to_location_type;

            req.body.pharmacy_stock_detail[i].sales_uom = req.body.pharmacy_stock_detail[i].uom_transferred_id;
    ...
    db.getConnection(function (error, connection) {
      connection.query("SELECT * from  hims_f_pharamcy_material_header \
        ...
        if (headerResult.length != 0) {
          ...
          connection.query("select * from hims_f_pharmacy_material_detail p left outer join hims_m_item_location l \
                on l.item_id =p.item_id where pharmacy_header_id=? and l.record_status='A'and l.pharmacy_location_id=? \
                and l.expirydt > now() and l.qtyhand>0  order by l.expirydt asc limit 0,1", [headerResult[0].hims_f_pharamcy_material_header_id, headerResult[0].to_location_id], function (error, pharmacy_stock_detail) {
            if (error) {
              (0, _utils.releaseDBConnection)(db, connection);
              next(error);
            }
