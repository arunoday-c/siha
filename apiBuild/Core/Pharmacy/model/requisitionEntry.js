...
var addrequisitionEntry = function addrequisitionEntry(req, res, next) {
  try {
    if (req.db == null) {
    ...
    db.getConnection(function (error, connection) {
      ...
      connection.beginTransaction(function (error) {
        ...
        return new _bluebird2.default(function (resolve, reject) {
          ...
          connection.query("INSERT INTO `hims_f_pharamcy_material_header` (material_requisition_number,requistion_date,from_location_type,\
            ...
            if (headerResult.insertId != null) {
              ...
              connection.query("INSERT INTO hims_f_pharmacy_material_detail(" + insurtColumns.join(",") + ",pharmacy_header_id) VALUES ?", [(0, _utils.jsonArrayToObject)({
...
var getrequisitionEntry = function getrequisitionEntry(req, res, next) {
  var selectWhere = {
    ...
    db.getConnection(function (error, connection) {
      connection.query("SELECT * from  hims_f_pharamcy_material_header\
        ...
        if (headerResult.length != 0) {
          ...
          connection.query("select * from hims_f_pharmacy_material_detail where pharmacy_header_id=?", headerResult[0].hims_f_pharamcy_material_header_id, function (error, pharmacy_stock_detail) {
            if (error) {
              (0, _utils.releaseDBConnection)(db, connection);
              next(error);
            }
...
var updaterequisitionEntry = function updaterequisitionEntry(req, res, next) {
  ...
  db.getConnection(function (error, connection) {
    ...
    connection.beginTransaction(function (error) {
      ...
      connection.query("UPDATE `hims_f_pharamcy_material_header` SET `authorize1`=?, `authorize1_date`=?, `authorize1_by`=?, \
      `authorie2`=?, `authorize2_date`=?, `authorize2_by`=? \
      WHERE `hims_f_pharamcy_material_header_id`=?", [inputParam.authorize1, new Date(), inputParam.updated_by, inputParam.authorie2, new Date(), inputParam.updated_by, inputParam.hims_f_pharamcy_material_header_id], function (error, result) {
        if (error) {
          connection.rollback(function () {
            (0, _utils.releaseDBConnection)(db, connection);
            next(error);
        ...
        if (result !== "" && result != null) {
          ...
          for (var i = 0; i < details.length; i++) {
            qry += " UPDATE `hims_f_pharmacy_material_detail` SET pharmacy_header_id='" + details[i].pharmacy_header_id + "',completed='" + details[i].completed + "',item_category_id='" + details[i].item_category_id + "',item_group_id='" + details[i].item_group_id + "',item_id='" + details[i].item_id + "',quantity_required='" + details[i].quantity_required + "',quantity_authorized='" + details[i].quantity_authorized + "',item_uom='" + details[i].item_uom + "',quantity_recieved='" + (details[i].quantity_recieved || 0) + "',quantity_outstanding='" + (details[i].quantity_outstanding || 0) +
            // "',po_created_date='" +
            ...
            "' WHERE hims_f_pharmacy_material_detail_id='" + details[i].hims_f_pharmacy_material_detail_id + "';";
          }
...
//created by Nowshad: to get Pharmacy Requisition Entry
var getAuthrequisitionList = function getAuthrequisitionList(req, res, next) {
  var selectWhere = {
  ...
  try {
    ...
    var strQuery = "SELECT * from  hims_f_pharamcy_material_header\
    where cancelled='N' ";
...
var updaterequisitionEntryOnceTranfer = function updaterequisitionEntryOnceTranfer(req, res, next) {
  if (req.db == null) {
    next(_httpStatus2.default.dataBaseNotInitilizedError());
  }
  var db = req.db;
  ...
  var partial_recived = new _nodeLinq.LINQ(inputParam.pharmacy_stock_detail).Where(function (w) {
  ...
  (0, _logging.debugLog)("complete: ", complete);
  connection.query("UPDATE `hims_f_pharamcy_material_header` SET `is_completed`=?, `completed_date`=? \
      WHERE `hims_f_pharamcy_material_header_id`=?", [complete, new Date(), inputParam.hims_f_pharamcy_material_header_id], function (error, result) {
    if (error) {
      connection.rollback(function () {
        (0, _utils.releaseDBConnection)(db, connection);
        next(error);
    ...
    if (result != "" && result != null) {
      ...
      for (var i = 0; i < details.length; i++) {
        qry += " UPDATE `hims_f_pharmacy_material_detail` SET quantity_outstanding='" + details[i].quantity_outstanding + "' WHERE hims_f_pharmacy_material_detail_id='" + details[i].material_requisition_detail_id + "';";
      }
...
module.exports = {
  addrequisitionEntry: addrequisitionEntry,
  getrequisitionEntry: getrequisitionEntry,
  updaterequisitionEntry: updaterequisitionEntry,
  getAuthrequisitionList: getAuthrequisitionList,
  updaterequisitionEntryOnceTranfer: updaterequisitionEntryOnceTranfer
};
