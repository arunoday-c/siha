...
export default function WardBedSetup(Props: any) {
  ...
  const [wardHeaderRow, setWardHeaderRow] = useState([]);
  ...
  const updateWardDetails = async (data: any) => {
    ...
    setWardDetailsData((pre: any) => {
      ...
      const row: { rIndex: number } = wardRow;
      let updateItemObj = {
        bed_id: data.bed_id,
        bed_no: data.bed_no,
      };
      details[row.rIndex] = { ...wardRow, ...updateItemObj };
  ...
  const wardHeader = (data: any) => {
    debugger;
    if (wardHeaderRow.length > 0) {
      updateWardHeader(data);
    } else {
      addWardHeader(data);
    }
  };
                        ...
                        columns={[
                          ...
                          },
                          {
                            ...
                            displayTemplate: (row: any) => {
                              const display: Array<{
                                bed_desc: string;
                              }> = bedDropDown.filter((f: any) => {
                                return (
                                  f.hims_adm_ip_bed_id === parseInt(row.bed_id)
                                );
                              });
                              return display[0].bed_desc;
                            },
                          },
                          {
                            fieldName: "bed_no",
