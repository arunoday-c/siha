    ...
    try {
      ...
      options.mysql
        .executeQuery({
          ...
          where SI.sales_invoice_mode='I' and date(SI.invoice_date) between date(?) and date(?) ${strQuery} order by SI.sales_invoice_mode desc, DNB.tax_percentage; 
          ...
          inner join hims_d_customer CUS on CUS.hims_d_customer_id = SI.customer_id
          where SI.sales_invoice_mode='S' and date(SI.invoice_date) between date(?) and date(?) ${strQuery} order by SI.sales_invoice_mode desc, SIS.tax_percentage;`,
          values: [
            input.from_date,
            input.to_date,
            input.from_date,
