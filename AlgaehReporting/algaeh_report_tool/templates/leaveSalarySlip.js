    ...
    try {
      ...
      options.mysql
        .executeQuery({
          ...
          S.absent_days,S.total_work_days,S.total_weekoff_days,S.total_holidays,S.total_leave, S.paid_leave,S.unpaid_leave, \
          S.total_paid_days,S.pending_unpaid_leave,SE.amount as earning_amount,EDD.hims_d_earning_deduction_id as deduction_id, \
          EDD.earning_deduction_description as deduction_description,SD.amount as deduction_amount, S.total_earnings,S.total_deductions,\ 
          SDP.sub_department_name, D.department_name, E.employee_code, E.full_name,E.arabic_name, E.date_of_joining, \ 
          E.exit_date,DE.designation,H.hospital_name, COALESCE(GP.payable_amount,0) as gratuity_amount, \ 
          COALESCE(LS.balance_leave_days,0) as annual_leave_days, COALESCE(LS.balance_leave_salary_amount,0) as annual_leave_salary_amount, \ 
          COALESCE(LE.leave_days,0) as encashed_leave_days, COALESCE(LE.leave_amount,0) as encashed_leave_amount, \ 
          S.loan_due_amount, LO.loan_due_amount, (COALESCE(S.loan_due_amount,0) + COALESCE(LO.loan_due_amount,0)) as total_loan_due, \ 
          (COALESCE(S.net_salary,0) + COALESCE(LO.loan_due_amount,0)) as net_before_loan
          ...
          left join hims_f_salary_loans LO on LO.salary_header_id = S.hims_f_salary_id 
          ...
          if (result.length > 0) {
            ...
            employees.forEach((employe) => {
              ...
              outputArray.push({
                ...
                total_loan_due: options.currencyFormat(
                  employe[0].total_loan_due,
                  options.args.crypto
                ),
                net_before_loan: options.currencyFormat(
                   employe[0].net_before_loan,
                  options.args.crypto
                ),
