    ...
    try {
      ...
      options.mysql
        .executeQuery({
          ...
          left join hims_f_patient_visit V on P.hims_d_patient_id = V.patient_id
          ...
          left join hims_f_lab_sample LS on LO.hims_f_lab_order_id = LS.order_id
          left join hims_f_ord_analytes OA on LO.hims_f_lab_order_id = OA.order_id
          left join hims_d_lab_specimen MS on LS.sample_id = MS.hims_d_lab_specimen_id
          left join hims_d_lab_analytes LA on OA.analyte_id = LA.hims_d_lab_analytes_id
          left join hims_d_services S on S.hims_d_services_id= LO.service_id
          left join algaeh_d_app_user U on LO.validated_by=U.algaeh_d_app_user_id
          left join hims_d_employee E on  U.employee_id=E.hims_d_employee_id
          left join hims_d_investigation_test IT on IT.services_id= LO.service_id
          left join hims_d_test_category TC on TC.hims_d_test_category_id= IT.category_id
          left join hims_m_lab_analyte LM on LM.analyte_id = LA.hims_d_lab_analytes_id and LM.test_id = LO.test_id
