    ...
    try {
      ...
      options.mysql
        .executeQuery({
          ...
          BD.gross_amount, BD.patient_resp as patient_share,	BD.total_tax,	  
          BD.patient_payable,	coalesce(BD.discount_amout,	0) as discount_amount, coalesce(BD.net_amout,	0) as net_amount,(BD.net_amout + BD.total_tax) as total_net_amount,
                ...
                BH.receiveable_amount,coalesce(BH.balance_credit,	0) as balance_credit 
        ...
        .then((ress) => {
          ...
          const result = {
            ...
            total_patient_payable: options.currencyFormat(
              _.sumBy(
                detail,
                (s) => parseFloat(s.net_amount) + parseFloat(s.total_tax)
              ),
              userObject,
              false
            ),
            ...
            total_patient_tax: options.currencyFormat(
              _.sumBy(detail, (s) => parseFloat(s.total_tax)),
              userObject,
              false
            ),
            ...
            total_balance_amount: options.currencyFormat(
              parseFloat(_.head(detail).balance_credit) +
                _.sumBy(detail, (s) => parseFloat(s.net_claim)),
              userObject,
              false
            ),
