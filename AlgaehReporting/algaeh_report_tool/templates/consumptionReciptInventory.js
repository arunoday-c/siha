...
const executePDF = function executePDFMethod(options) {
	return new Promise(function(resolve, reject) {
		try {
          ...
          date(?) and date(?)	${str};`,
					values: [ input.sub_department_id, input.hospital_id, input.from_date, input.to_date ],
					printQuery: true
				})
				.then((results) => {
					...
					const result = _.chain(results)
						...
						.value();
					const grandTotal = _.chain(result).sumBy((s) => parseFloat(s.total_cost)).value().toFixed(2);
