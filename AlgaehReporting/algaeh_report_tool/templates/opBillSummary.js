      ...
      options.mysql
        .executeQuery({
          query: `select BH.hims_f_billing_header_id,BH.bill_date,BD.hims_f_billing_details_id,BD.service_type_id, ST.service_type_code,ST.service_type, sum(BD.net_amout)as total_amount 
			...
			where BH.hospital_id=? and  BH.record_status='A' and BD.cancel_yes_no='N' and BH.adjusted='N' and BD.record_status='A' and date(BH.bill_date) between date(?) and date(?)  group by BD.service_type_id `,
