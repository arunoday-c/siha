      ...
      options.mysql
        .executeQuery({
          query: `select hims_d_earning_deduction_id,earning_deduction_description,short_desc,component_category, print_order_by, \
				...
				S.hims_f_salary_id,S.salary_number,S.salary_date,S.present_days,S.total_days,S.display_present_days,S.total_paid_days,S.net_salary,S.total_earnings,S.total_deductions,S.loan_due_amount,S.salary_paid_date, case when S.salary_paid='Y' then 'Paid' else 'Unpaid' end as payment_status,
case when S.salary_processed='Y' then 'Finalized' else 'Not Finalized' end as processed_status,\
        ...
        coalesce(S.ot_holiday_hours,0.0) as ot_holiday_hours,H.hospital_name,SD.sub_department_name
				...
				where E.suspend_salary ='N' and S.salary_type ='NS' and E.employee_status='A' and E.employee_group_id=? and S.month=? and S.year=?  ${is_local} ${str}`,
          values: [input.employee_group_id, input.month, input.year],
          printQuery: true,
        })
        .then((result) => {
