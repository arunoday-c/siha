...
const executePDF = function executePDFMethod(options) {
  return new Promise(function (resolve, reject) {
    try {
      ...
      options.mysql
        .executeQuery({
          query: `SELECT BH.bill_number as doc_number,BH.bill_date as bill_date, V.visit_date, P.hims_d_patient_id as patient_id,P.full_name, 
          P.patient_code,  N.nationality,
          CASE WHEN BD.insurance_yesno='Y' THEN 'Insurance' else 'Cash' END as insurance_yesno,"Billing" as data_from, 
          BD.net_amout as total_before_vat,BD.total_tax,BD.patient_tax,BD.company_tax,(coalesce(BD.patient_payable,0)+coalesce(BD.company_payble,0)) as total_after_vat, 
          S.service_name,BH.hims_f_billing_header_id as bill_id, SI.insurance_sub_name as company_name
            ...
            P.full_name, P.patient_code, N.nationality,
            CASE WHEN PD.insurance_yesno='Y' THEN 'Insurance' else 'Cash' END as insurance_yesno,"Pharmacy" as data_from,
            ...
            left join hims_d_services S on PD.service_id=S.hims_d_services_id
          ...
          const BillWise = _.chain(final_result)
            ...
            .map((billNo) => {
              ...
              return {
                ...
                nationality,
                company_name,
                total_before_vat: _.sumBy(billNo, (s) =>
                  parseFloat(s.total_before_vat)
                ),
                total_tax: _.sumBy(billNo, (s) => parseFloat(s.total_tax)),
                total_after_vat: _.sumBy(billNo, (s) =>
                  parseFloat(s.total_after_vat)
                ),
                patient_tax: _.sumBy(billNo, (s) => parseFloat(s.patient_tax)),
                company_tax: _.sumBy(billNo, (s) => parseFloat(s.company_tax)),
              };
            })
            .value();
          resolve({
