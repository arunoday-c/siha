  ...
  return new Promise(function (resolve, reject) {
    ...
    try {
      ...
      if (input.location_id > 0) {
        str += ` and PTH.from_location_id = ${input.location_id}`;
      }
      ...
      options.mysql
        .executeQuery({
          query:
            "SELECT PTH.batchno, PTH.expiry_date, sum(PTH.transaction_qty) as transaction_qty, \
            IM.item_code,IC.category_desc,IM.category_id, IM.item_description, \
            IU.uom_description, PTH.transaction_total,INVL.location_description, CH.consumption_date, \
            CH.consumption_number, PTH.transaction_type FROM hims_f_inventory_trans_history PTH \
            ...
            inner join hims_f_inventory_consumption_header CH on CH.hims_f_inventory_consumption_header_id = PTH.transaction_id \
