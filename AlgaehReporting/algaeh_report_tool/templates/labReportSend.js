...
inner join algaeh_d_app_user CL on CL.algaeh_d_app_user_id = LS.collected_by
inner join hims_d_employee CLE on CLE.hims_d_employee_id = CL.employee_id
inner join algaeh_d_app_user CN on CN.algaeh_d_app_user_id = LO.created_by
inner join hims_d_employee CNE on CNE.hims_d_employee_id = CN.employee_id
inner join algaeh_d_app_user VL on VL.algaeh_d_app_user_id = LO.validated_by
inner join hims_d_employee VLE on VLE.hims_d_employee_id = VL.employee_id
where LO.billed='Y' and date(LO.ordered_date) between date(?) and date(?) and LO.hospital_id=? and LO.send_out_test = ? ; `,
            values: [ input.from_date, input.to_date,input.hospital_id,input.is_SendOut],
            printQuery: true,
          })
          .then((res) => {
