  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      options.mysql
        .executeQuery({
          query:
            `SELECT TH.* , PU.uom_description,PM.item_description,PL.location_description from hims_f_pharmacy_trans_history TH 
              left join hims_d_item_master  PM on TH.item_code_id=PM.hims_d_item_master_id
              left join  hims_d_pharmacy_location PL on TH.from_location_id = PL. hims_d_pharmacy_location_id
              left join hims_d_pharmacy_uom PU on TH.transaction_uom=PU.hims_d_pharmacy_uom_id
