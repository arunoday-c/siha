...
export function PlotUI(node, style, indexIds, expand) {
  function PlotUIInner(node, style, indexIds) {
    if (!hasChildren(node)) {
      return (
        <li
        ...
        >
          ...
          >
            {node.children.map((innerNode, index) => {
              ...
              newIndex.push(index);
              return PlotUIInner(innerNode, style, newIndex, expand);
            })}
          </ul>
        </li>
      );
  ...
  return PlotUIInner(node, style, indexIds);
}
