...
import {
    ComposedChart, Line,ResponsiveContainer, Bar, XAxis, YAxis, CartesianGrid, Tooltip,
    Legend,
} from 'recharts';
...
export default  memo(function (props) {
 const {data,xAxis,yAxisBar,yAxisLine} = props;
    return (<div style={{ width: '100%', height: 230 }}>
            {data.length ===0?(<Empty image={(<Icon type="bar-chart" style={{ fontSize: '115px', color: '#08c' }}  />)} />):(<ResponsiveContainer ><ComposedChart
                width={500}
                height={400}
                data={data}
                margin={{
