    ...
    if (option.old == "Y") {
      ...
      _mysql
        ...
        .then((optionRecords) => {
          ...
          const current_from_date = `${current_year}${_start_month}${_start_date}`;
          const _end_month =
            String(end_month).length === 1 ? "0" + end_month : end_month;
          const _end_date =
            String(end_date).length === 1 ? "0" + end_date : end_date;
          const current_to_date = `${current_year}${_end_month}${_end_date}`;
          const next_from_date = `${next_year}${_start_month}${_start_date}`;
          const next_to_date = `${next_year}${_end_month}${_end_date}`;
          option["current_from_date"] = current_from_date;
          option["current_to_date"] = current_to_date;
  ...
  getAccountReceivableAging: (req, res, next) => {
    ...
    _mysql
      ...
      .then((result) => {
        ...
        let interval = parseInt(req.query.interval) + 1;
        let dis_interval = parseInt(req.query.interval);
        let period = parseInt(req.query.period) - 3; //period
        let dyn_interval = { Current: "", [`1-${dis_interval}`]: "" };
        ...
        //     head_id in(${head_ids}) and settlement_status='P' and D.auth_status='A' and date(H.due_date) <=date('${req.query.till_date}')
        //     group by child_id  with rollup;`;
        // } else {
        //Current and 1st day to give interval
        strQuery = ` select   'Current' as days, ROUND(  sum(debit_amount)-sum(H.settled_amount), ${decimal_places}) as debit_amount ,child_id from             
                finance_voucher_header H inner join finance_voucher_details D on H.finance_voucher_header_id=D.voucher_header_id where
                head_id in(${head_ids}) and settlement_status='P' and D.auth_status='A' and DATE(H.due_date) =date('${req.query.till_date}')
                group by child_id  with rollup;
              select  '1-${dis_interval}' as days, ROUND(sum(debit_amount)-sum(H.settled_amount), ${decimal_places}) as debit_amount ,child_id from 
                finance_voucher_header H inner join finance_voucher_details D  on H.finance_voucher_header_id=D.voucher_header_id where
                head_id in(${head_ids}) and settlement_status='P' and D.auth_status='A' and H.due_date between  
                DATE_SUB(date('${req.query.till_date}'),INTERVAL ${interval} DAY)  and DATE_SUB(date('${req.query.till_date}'),INTERVAL 1 DAY)  group by child_id  with rollup;`;

        //Rest interval based on period
        for (let i = 0; i < period; i++) {
          //period
          dyn_interval[
            `${dis_interval}-${dis_interval + parseInt(req.query.interval)}`
          ] = "";
          strQuery += `select '${dis_interval}-${
            dis_interval + parseInt(req.query.interval)
          }' as days, ROUND(sum(debit_amount)-sum(H.settled_amount), ${decimal_places}) as debit_amount ,child_id from 
            finance_voucher_header H inner join finance_voucher_details D  on H.finance_voucher_header_id=D.voucher_header_id where
            head_id in(${head_ids}) and settlement_status='P' and D.auth_status='A' and H.due_date between  
            DATE_SUB(date('${req.query.till_date}'),INTERVAL ${
            interval + 1 + parseInt(req.query.interval)
          } DAY)  and DATE_SUB(date('${req.query.till_date}'),INTERVAL ${
            interval + 1
          }  DAY)  group by child_id  with rollup;`;

          interval = interval + 1 + parseInt(req.query.interval);
          dis_interval = dis_interval + parseInt(req.query.interval);
        }
        //Last record
        ...
        _mysql
          ...
          .then((Result) => {
            ...
            // console.log("2", ledgers);
            ledgers.forEach((ledger) => {
              // let bef_out_array=[]
              let bef_out_obj = { customer: ledger.child_name };
              ...
              for (let i = 1; i < Result.length; i++) {
                ...
                // );
                // if (ledger.finance_account_child_id === 630) {
                //   console.log("Result[i]", Result[i]);
                //   console.log(
                //     "finance_account_child_id",
                //     ledger.finance_account_child_id
                ...
                // }
                // console.log(
                //   "finance_account_child_id" + i,
                //   ledger.finance_account_child_id
                // );

                if (Result[i].length > 0) {
                  const _data = Result[i].filter(
                    (f) => f.child_id == ledger.finance_account_child_id
                  );

                  // console.log("_data", _data);

                  Object.keys(dyn_interval).map((item) => {
                    // console.log("item", item);
                    const final_data = _data.find((f) => f.days === item);
                    // if (ledger.finance_account_child_id === 485) {
                    //   console.log("item", item);
                    //   console.log("final_data", final_data);
                    //   console.log("bef_out_obj_new:", bef_out_obj);
                    // }
                    if (final_data !== undefined && item === final_data.days) {
                      bef_out_obj[item] = parseFloat(
                        final_data.debit_amount
                      ).toFixed(decimal_places);
              ...
              // let above_ninety_days_amount =
              //   parseFloat(0).toFixed(decimal_places);
  ...
  getAccountPayableAging: (req, res, next) => {
    ...
    _mysql
      ...
      .then((result) => {
        ...
        //Current and 1st day to give interval
        strQuery = `
           select 'Current' as days, ROUND(  sum(credit_amount)-sum(H.settled_amount), ${decimal_places}) as credit_amount ,child_id from
          finance_voucher_header H inner join finance_voucher_details D on H.finance_voucher_header_id=D.voucher_header_id where
          head_id in(${head_ids}) and settlement_status='P' and D.auth_status='A' and H.due_date =date('${req.query.till_date}')
          group by child_id  with rollup;
          select  '1-${dis_interval}' as days, ROUND(sum(credit_amount)-sum(H.settled_amount), ${decimal_places}) as credit_amount ,child_id from
          finance_voucher_header H inner join finance_voucher_details D  on H.finance_voucher_header_id=D.voucher_header_id where
          head_id in(${head_ids}) and settlement_status='P' and D.auth_status='A' and H.due_date between  
          DATE_SUB(date('${req.query.till_date}'),INTERVAL ${interval} DAY)  and DATE_SUB(date('${req.query.till_date}'),INTERVAL 1 DAY) 
           group by child_id  with rollup;`;
        ...
        for (let i = 0; i < period; i++) {
          ...
          strQuery += `select '${dis_interval}-${
            dis_interval + parseInt(req.query.interval)
          }' as days, ROUND(sum(credit_amount)-sum(H.settled_amount) , ${decimal_places}) as credit_amount ,child_id from
            finance_voucher_header H inner join finance_voucher_details D on H.finance_voucher_header_id=D.voucher_header_id where
            head_id in(${head_ids}) and settlement_status='P' and D.auth_status='A' and H.due_date between 
              DATE_SUB(date('${req.query.till_date}'),INTERVAL ${
            interval + 1 + parseInt(req.query.interval)
              ...
              for (let i = 1; i < Result.length; i++) {
                ...
                if (Result[i].length > 0) {
                  ...
                  Object.keys(dyn_interval).map((item) => {
                    ...
                    if (final_data !== undefined && item === final_data.days) {
                      bef_out_obj[item] = parseFloat(
                        final_data.credit_amount
                      ).toFixed(decimal_places);

                      balance = balance + parseFloat(final_data.credit_amount);
                    }
        ...
        const execute = () => {
          if (trans_symbol == "Dr") {
            ...
            if (
              ...
              op_amount = parseFloat(SUM_CREDIT_DEBIT).toFixed(decimal_places);
            }
          }
        };
        if (finance_account_head_id === 4 || finance_account_head_id === 5) {
