...
export default {
  ...
  //created by:irfan
  /** Same service can use for get insurance by resubmission  */
  getInvoicesForClaims: (req, res, next) => {
    const _mysql = new algaehMysql();
    try {
      let input = req.query;
      const { insurance_statement_id } = input;
      let _qryStr = "";
      const _values = [];
      let queryValue = [];
      let differenceStatement = " IH.insurance_statement_id IS NULL ";
      if (insurance_statement_id === undefined) {
        if (
          input.patient_id > 0 ||
          ...
          queryValue = [..._values, ..._values, ..._values];
        } else {
          req.records = {
            invalid_data: true,
      ...
      _mysql
        .executeQuery({
          query:
            "SELECT 0 chkselect, hims_f_invoice_header_id,AU.algaeh_d_app_user_id,AUU.user_display_name,IH.requested_by,IH.request_comment, invoice_number, invoice_date, IH.patient_id, IH.claim_status, visit_id,\
          ...
          P.patient_code,P.full_name as patient_name,P.primary_id_no,P.arabic_name as arabic_patient_name,P.contact_number ,\
          V.visit_code,date(V.visit_date) as visit_date,V.episode_id,V.doctor_id,E.full_name as doctor_name,E.employee_code,insurance_provider_name,\
          ...
          NET_OF.price_from,NET_OF.employer,NET_OF.policy_number,SD.department_type\
            ...
            "; \
             ...
             ID.company_resp, ID.company_tax, ID.company_payable, ID.sec_company_resp, ID.sec_company_tax, ID.sec_company_payable,\
              ...
              on IH.hims_f_invoice_header_id =ID.invoice_header_id inner join hims_d_service_type ST on \
               ...
               ST.record_status='A'  and S.record_status='A' and  " +
            differenceStatement +
            _qryStr +
    ...
    try {
      ...
      _mysql
        .executeQuery({
          ...
          INH.request_comment,USR.user_display_name,INH.correction_requested,E.full_name as doctorName, 
          ...
          left join algaeh_d_app_user USR on USR.algaeh_d_app_user_id=INH.requested_by
