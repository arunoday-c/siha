  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        newData.push({
          hims_d_employee_id,
          primary_user_ids: reporting_to_id,
          primary_message: `Employee <b>${english_name}</b> / ${employee_code} is going to complete probationary period  - (${probation_date}) with in ${expire_after} days.`,
          other_user_ids: employees,
          other_message: `Employee <b>${english_name}</b> / ${employee_code} is going to complete probationary period  - (${probation_date}) with in ${expire_after} days.`,
        });
      }
