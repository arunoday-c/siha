...
let algaehSearchConfig = (searchName, req) => {
         ...
         where record_status ='A' and hospital_id=" +
          ...
          where PH.record_status ='A' and PH.posted='Y' and PH.hospital_id=" +
			...
			},
			{
				searchName: 'REQEntry',
				searchQuery:
					'select SQL_CALC_FOUND_ROWS * from hims_f_pharamcy_material_header and hospital_id=' + hospitalId,
          ...
          and is_completed = 'N' and requistion_type='PR' and hospital_id=" +
			...
			},
			{
				searchName: 'POEntry',
				searchQuery:
					'select SQL_CALC_FOUND_ROWS * from hims_f_procurement_po_header where hospital_id=' + hospitalId,
           ...
           and cancelled='N' and is_completed='N' and  hospital_id=" +
			...
			},
			{
				searchName: 'DNEntry',
				searchQuery:
					'select SQL_CALC_FOUND_ROWS * from hims_f_procurement_dn_header where hospital_id=' + hospitalId,
			...
			},
			{
				...
				searchQuery:
					'select SQL_CALC_FOUND_ROWS * from hims_f_procurement_grn_header where hospital_id=' + hospitalId,
	...
	let row = new LINQ(queries.algaehSeach)
		.Where((w) => String(w.searchName).toUpperCase() === String(searchName).toUpperCase())
		.FirstOrDefault();
	return row;
};
