...
let searchData = (req, res, next) => {
	...
	try {
		...
		if (inputParam.searchName == null || inputParam.searchName == '')
			next(httpStatus.generateError(httpStatus.badRequest, 'Please provide correct search details'));
		let queryConfig = algaehSearchConfig(inputParam.searchName, req);
		if (queryConfig == null) {
			next(httpStatus.generateError(httpStatus.unAuthorized, 'No access for this content'));
		}

		let limit = req.query.pageSize == null || req.query.pageSize === 0 ? 5 : req.query.pageSize;
		//let offSet = req.query.pageNo === 0 ? limit * 1 : limit * req.query.pageNo;
		let offSet = req.query.pageNo;
		let whereCondition =
			req.query.fieldName == null
				? ' '
				: ' and upper(' + req.query.fieldName + ") like  upper('%" + req.query.fieldContains + "%')";

		const _groupby = queryConfig.groupBy != null ? ' ' + queryConfig.groupBy + ' ' : '';

		whereCondition += req.query.inputs == 'null' || null ? '' : ' and ' + req.query.inputs;
		let query =
...
const newSearch = (req, res, next) => {
	...
	let limit = inputParam.pageSize == null || inputParam.pageSize === 0 ? 10 : inputParam.pageSize;
	let offSet = inputParam.pageNo;
	let _hasQuery = queryConfig.searchQuery;
	if (!String(_hasQuery).toUpperCase().includes('WHERE')) {
		_hasQuery += ' WHERE ';
	}
	...
	if (String(_hasQuery).toLowerCase().includes('{mapper}')) {
		let mapper =
			'(' +
			_.map(sortedParameters, (items) => {
				return _.keysIn(items).map((key) => {
					// _values.push("%" + items[key] + "%");
					return String('UPPER(`' + key + '`)') + " LIKE UPPER('%" + items[key] + "%')";
				});
			}).join(' OR ') +
			')';
