...
let apiAuth = (req, res, next) => {
	...
	try {
		...
		if (!authHeader || authHeader == '') {
			next(httpStatus.generateError(httpStatus.unAuthorized, 'Missing authorization token'));
		}

		let inputData = extend(authModel, getUserNamePassWord(authHeader));

		if (inputData.username == null || inputData.username == '') {
			next(httpStatus.generateError(httpStatus.badRequest, 'User name can not blank'));
		}
		if (inputData.password == null || inputData.password == '') {
			next(httpStatus.generateError(httpStatus.badRequest, 'Password can not blank'));
		}
		req.body = inputData;
		_mysql
			.executeQuery({
...
let authUserOLD = (req, res, next) => {
	...
	try {
      ...
      AND algaeh_m_group_user_mappings.user_id=algaeh_d_app_user.algaeh_d_app_user_id";
			...
			connection.query(query, [ inputData.password, inputData.username ], (error, result) => {
				if (error) {
					connection.release();
					next(error);
				}
...
let authUser = (req, res, next) => {
   ...
   hims_d_hospital, hims_d_currency CUR WHERE hims_d_hospital.record_status='A' AND \
   CUR.hims_d_currency_id=default_currency AND hims_d_hospital_id=? ",
				values: [ inputData.password, inputData.username, inputData.item_id, inputData.item_id ],
				printQuery: true
			})
			.then((result) => {
				_mysql.releaseConnection();
