...
export function getDentalOrder(req, res, next) {
...
export function addDentalOrder(req, res, next) {
  const _mysql = new algaehMysql();
  ...
  try {
    ...
    req.on("end", () => {
      ...
      // utilities.logger().log("addDentalOrder: ");
...
export function postDentalOrder(req, res, next) {
  const _mysql = new algaehMysql();
      ...
      .then((headerResult) => {
        if (headerResult.length != 0) {
          ...
          if (req.query.sales_order_mode == "I") {
            strQuery = mysql.format(
              "select QI.*,(QI.net_extended_cost + QI.tax_amount) as total_amount, IM.item_description, IU.uom_description, quantity as quantity_outstanding \
...
export function getDentalOrderList(req, res, next) {
  const _mysql = new algaehMysql();
  // const utilities = new algaehUtilities();
  try {
    ...
    _mysql
      .executeQuery({
        query:
          "SELECT SO.*, C.customer_name, IH.invoice_number,EM.full_name as created_by from hims_f_sales_order SO \
                inner join hims_d_customer C  on SO.customer_id = C.hims_d_customer_id \
...
export function updateDentalOrderEntry(req, res, next) {
  const _mysql = new algaehMysql();
...
export function getContractDentalOrder(req, res, next) {
  const _mysql = new algaehMysql();
  try {
