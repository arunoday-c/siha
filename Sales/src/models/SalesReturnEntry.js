...
export function getSalesReturn(req, res, next) {
  ...
  try {
    ...
    _mysql
      .executeQuery({
        query:
          "SELECT H.*, C.customer_name,HS.hospital_name, IH.invoice_number, P.project_desc as project_name, \
      ...
      .then((headerResult) => {
        if (headerResult.length != 0) {
          _mysql
            .executeQuery({
              query:
                "select QI.*, IM.item_description, IU.uom_description, IM.sales_uom_id from hims_f_sales_return_detail QI \
    ...
    _mysql
      .executeQuery({
        query:
          "SELECT *, SIH.net_payable as invoice_net_payable, SIH.net_payable as return_total, \
                    ...
                    L.location_type, SIH.location_id FROM hims_f_sales_invoice_header SIH \
                    ...
                    inner join  hims_d_inventory_location L on  SIH.location_id = L.hims_d_inventory_location_id \
      ...
      .then((headerResult) => {
        _mysql
          .executeQuery({
            query:
              "SELECT GD.dispatch_note_header_id, DNB.*, IM.item_description, IM.sales_uom_id, \
        ...
        _mysql
          ...
          .then((headerResult) => {
            ...
            _mysql
              .executeQuery({
                query:
                  "INSERT INTO hims_f_sales_return_detail(??) VALUES ? ;" +
                  strGrnQry,
          ...
          _mysql
            ...
            .then((result) => {
              ...
              _mysql
                .executeQuery({
                  query:
                    "select GH.hims_f_sales_return_header_id, GH.sales_return_number, IH.invoice_number, GH.net_total, GH.tax_amount, \
