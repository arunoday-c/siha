  ...
  (
   ...
   const userContext = useContext(MainContext);
    ...
    const attachRel = (data) => {
      ...
      if(data.allBranches=== undefined) return {};
    const {organization_code,organization_id,organization_name} =  userContext.userToken;
      const modifiedData = data.allBranches.map((item)=>{
...
...children
  ...
  //  console.log("data",data)
      // data.relationship =
      //   flags + (data.subDepts && data.subDepts.length > 0 ? 1 : 0);
      // if (data.subDepts) {
      //   data.subDepts.forEach(function(item) {
      //     attachRel(item, "1" + (data.subDepts.length > 1 ? 1 : 0));
      //   });
      // }
      
      return {id:organization_id,name:organization_code,title:organization_name, children:modifiedData};
    };
    const newData = attachRel(datasource)
    const attachRelationship = (newData,flags) => {
      
      newData.relationship =
        flags + (newData.children && newData.children.length > 0 ? 1 : 0);
      if (newData.children) {
        newData.children.forEach(function(item) {
          attachRelationship(item, "1" + (newData.children.length > 1 ? 1 : 0));
        });
      }
    ...
    return (
      ...
      >
        ...
        >
          <ul>
            <ChartNode
              datasource={attachRelationship(ds,"00")}
