...
const deleteTransEntryDetail = ($this, context, e, rowId) => {
	...
	let display =
		$this.props.itemlist === undefined
			? []
			: $this.props.itemlist.filter((f) => f.hims_d_inventory_item_master_id === e.item_id);
...
const onchangegridcol = ($this, context, row, e) => {
	...
	if (parseFloat(value) > parseFloat(row.qtyhand)) {
		...
		item_details.quantity_outstanding =
			item_details.quantity_authorized - item_details.transfer_to_date - quantity_transferred;
		...
		if (item_details.quantity_outstanding < 0) {
			...
			row[name] = 0;
			quantity_transferred = _.sumBy(item_details.batches, (s) => parseFloat(s.quantity_transfer));
			item_details.quantity_transferred = quantity_transferred;
...
const AddSelectedBatches = ($this, context) => {
	if (
		...
		if (context !== null) {
			...
			_stock_detail[_index].inventory_stock_detail = batches.map((item, index) => {
				return { ...item, ...details };
			});
...
const CloseItemBatch = ($this, context, e) => {
	let batchno = e !== undefined ? (e.selected === true ? e.batchno : $this.state.batchno) : $this.state.batchno;
	let expiry_date =
		e !== undefined
			? e.selected === true ? moment(e.expirydt)._d : $this.state.expiry_date
			: $this.state.expiry_date;

	let grn_no = e !== undefined ? (e.selected === true ? e.grnno : $this.state.grn_no) : $this.state.grn_no;
	let qtyhand = e !== undefined ? (e.selected === true ? e.qtyhand : $this.state.qtyhand) : $this.state.qtyhand;

	let unit_cost = e !== undefined ? (e.selected === true ? e.avgcost : $this.state.unit_cost) : $this.state.unit_cost;
