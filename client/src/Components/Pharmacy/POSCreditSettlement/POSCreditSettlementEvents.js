...
const GenerateReciept = ($this, callback) => {
	...
	if ($this.state.Cashchecked === false && $this.state.Cardchecked === false && $this.state.Checkchecked === false) {
		swalMessage({
	...
	if (!err) {
		if ($this.state.unbalanced_amount === 0) {
			GenerateReciept($this, (that) => {
				...
				let listOfinclude = Enumerable.from(Inputobj.criedtdetails).where((w) => w.include === 'Y').toArray();
					...
					onSuccess: (response) => {
						...
						if (response.data.success) {
							$this.setState({
								...
								hims_f_pos_credit_header_id: response.data.records.hims_f_pos_credit_header_id,
...
export { ClearData, Validations, getCashiersAndShiftMAP, PatientSearch, getCtrlCode, SavePosCreidt };
