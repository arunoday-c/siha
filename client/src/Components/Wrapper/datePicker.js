...
export default class DateHandler extends PureComponent {
	...
	onDayChange = (e) => {
		if (this.props.singleOutput === undefined) {
			if (this.props.events !== undefined && typeof this.props.events.onChange === 'function') {
				this.props.events.onChange(moment(e.target.value, config.formators.date)._d, e.target.name);
			}
		} else {
			if (this.props.events !== undefined && typeof this.props.events.onChange === 'function') {
		...
		const value =
			this.state.value !== undefined
				? this.state.value !== null
					...
					: ''
				: '';
		const name = this.props.textBox.name !== undefined ? { name: this.props.textBox.name } : {};
		const _required =
			this.props.label !== undefined
				? this.props.label.isImp !== undefined ? { required: this.props.label.isImp } : {}
				: {};
