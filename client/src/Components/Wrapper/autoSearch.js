...
export default class AlgaehAutoSearch extends Component {
	...
	onResultSelectHandler(e, { result }) {
		this.setState(
			...
			() => {
				if (typeof this.props.onClick === 'function') this.props.onClick(result, this.props.name);
			}
		);
	}
	...
	generateInputParm() {
		const { columns, extraParameters, searchName, directCondition } = this.props;
		...
		let _arrayParam = columns.map((f) => {
			if (f.exclude === undefined || f.exclude !== true) return { [f.fieldName]: value };
		});
		...
		// });
		const _directCondition = directCondition === undefined ? {} : { directCondition: directCondition };
	...
	serviceCall(data) {
		const _uri = this.props.uri !== undefined ? this.props.uri : '/gloabelSearch/newSearch';
		algaehApiCall({
			uri: _uri,
			data: data,
			method: 'POST',
	...
	render() {
		...
		const _fluid = this.props.fullWidth !== undefined ? { fluid: this.props.fullWidth } : {};

		return (
			<div className={this.props.div !== undefined ? this.props.div.className : null}>
				{this.renderLabel()}
				<Search
					loading={isLoading}
					results={results}
					...
					{...this.props.others}
				/>{' '}
				<span className="autoSearchClear" onClick={this.onClearHandler.bind(this)}>
