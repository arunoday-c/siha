...
export function ScreenAssignmentEvents() {
  return {
    ...
    changeComponent: ($this, data, e) => {
      ...
      let _sList = _.map(screen_list, (f) => {
        return f.componentList.map((item) => {
          return { ...item };
        });
      });
    ...
    changeScreen: ($this, data, e) => {
      ...
      const hasUncheckedState = newModule.find((f) => {
        return f.checked === false;
      });
      ...
      $this.setState({
        ...
        checkAll: hasUncheckedState !== undefined ? false : true,
      });
    },
