...
const SalesOrderSearch = ($this, e) => {
    AlgaehSearch({
        ...
        inputs: " sales_order_mode = '" + $this.state.sales_invoice_mode + "' and authorize1='Y' \
                    ...
                    onSuccess: response => {
                        ...
                        if (response.data.success) {
                            ...
                            if (data.invoice_entry_detail_item.length === 0) {
                                swalMessage({
                                    title: "Dispatch Not yet Done for this Order",
                                    type: "warning"
                                });
                            }
                            ...
                            data.sub_total = _.sumBy(data.invoice_entry_detail_item, s => parseFloat(s.sub_total));
                            data.discount_amount = _.sumBy(data.invoice_entry_detail_item, s => parseFloat(s.discount_amount));
                            data.net_total = _.sumBy(data.invoice_entry_detail_item, s => parseFloat(s.net_total));
                            data.total_tax = _.sumBy(data.invoice_entry_detail_item, s => parseFloat(s.total_tax));
                            data.net_payable = _.sumBy(data.invoice_entry_detail_item, s => parseFloat(s.net_payable));
