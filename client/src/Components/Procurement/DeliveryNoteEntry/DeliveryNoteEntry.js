...
class DeliveryNoteEntry extends Component {
	...
	render() {
		return (
			<div>
				<BreadCrumb
					title={<AlgaehLabel label={{ forceLabel: 'Delivery Note', align: 'ltr' }} />}
					...
					pageNavPath={[
						...
						{
							pageName: <AlgaehLabel label={{ forceLabel: 'Delivery Note', align: 'ltr' }} />
						}
					]}
					soptlightSearch={{
						label: <AlgaehLabel label={{ forceLabel: 'Delivery Number', returnText: true }} />,
                ...
                />*/}
								...
								>
									<span
										...
										style={{
											...
											marginTop: 26,
											paddingBottom: 0,
											pointerEvents: this.state.dataExitst === true ? 'none' : ''
										}}
								...
								<div className="col">
									...
									<h6>
										{this.state.dn_from ? this.state.dn_from === 'INV' ? (
											'Inventory'
										) : (
											'Pharmacy'
										) : (
								...
								<div className="col">
									...
									<h6>{this.state.location_name ? this.state.location_name : '------'}</h6>
								</div>
								<div className="col">
									<AlgaehLabel label={{ forceLabel: 'Vendor' }} />
									<h6>{this.state.vendor_name ? this.state.vendor_name : '------'}</h6>
								</div>

								<div className="col">
									<AlgaehLabel label={{ forceLabel: 'Payment Terms' }} />
									<h6>{this.state.payment_terms ? this.state.payment_terms : '0 days'}</h6>
								</div>
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(DeliveryNoteEntry));
