...
const vendortexthandle = ($this, e) => {
	if (e.value === undefined) {
		...
		let value = e.value || e.target.value;
		let ReqData = true;
		if ($this.state.pharmcy_location_id !== null || $this.state.inventory_location_id !== null) {
			ReqData = false;
		}
				...
				if (data !== null && data !== undefined) {
					...
					for (let i = 0; i < data.po_entry_detail.length; i++) {
						data.po_entry_detail[i].item_id =
							data.po_entry_detail[i].phar_item_id || data.po_entry_detail[i].inv_item_id;

						data.po_entry_detail[i].po_quantity = data.po_entry_detail[i].authorize_quantity;

						data.po_entry_detail[i].dn_quantity = 0;

						data.po_entry_detail[i].quantity_recieved_todate =
							data.po_entry_detail[i].authorize_quantity - data.po_entry_detail[i].quantity_outstanding;

						data.po_entry_detail[i].authorize_quantity = 0;
						data.po_entry_detail[i].quantity_outstanding = 0;

						data.po_entry_detail[i].discount_percentage = data.po_entry_detail[i].sub_discount_percentage;

						data.po_entry_detail[i].discount_amount = data.po_entry_detail[i].sub_discount_amount;

						data.po_entry_detail[i].purchase_order_header_id = data.hims_f_procurement_po_header_id;
					...
					if ($this.props.purchase_number !== undefined && $this.props.purchase_number.length !== 0) {
...
const SaveDNEnrty = ($this) => {
	...
	for (var j = 0; j < InputObj.po_entry_detail.length; j++) {
		if ($this.state.dn_from === 'PHR') {
			if (InputObj.pharmacy_stock_detail === undefined || InputObj.pharmacy_stock_detail.length === 0) {
				InputObj.pharmacy_stock_detail = InputObj.po_entry_detail[j].dn_entry_detail;
			} else {
			...
			if (InputObj.inventory_stock_detail === undefined || InputObj.inventory_stock_detail.length === 0) {
				InputObj.inventory_stock_detail = InputObj.po_entry_detail[j].dn_entry_detail;
			} else {
	...
	if (InputObj.dn_from === 'PHR') {
		for (let i = 0; i < InputObj.pharmacy_stock_detail.length; i++) {
			InputObj.pharmacy_stock_detail[i].location_id = InputObj.pharmcy_location_id;
			...
			InputObj.pharmacy_stock_detail[i].uom_id = InputObj.pharmacy_stock_detail[i].pharmacy_uom_id;

			InputObj.pharmacy_stock_detail[i].sales_uom = InputObj.pharmacy_stock_detail[i].sales_uom_id;
			InputObj.pharmacy_stock_detail[i].item_id = InputObj.pharmacy_stock_detail[i].phar_item_id;
			InputObj.pharmacy_stock_detail[i].item_code_id = InputObj.pharmacy_stock_detail[i].phar_item_id;
			InputObj.pharmacy_stock_detail[i].grn_number = InputObj.grn_number;
			InputObj.pharmacy_stock_detail[i].item_category_id = InputObj.pharmacy_stock_detail[i].phar_item_category;
			InputObj.pharmacy_stock_detail[i].item_group_id = InputObj.pharmacy_stock_detail[i].phar_item_group;

			InputObj.pharmacy_stock_detail[i].net_total = InputObj.pharmacy_stock_detail[i].net_extended_cost;
		...
		for (let i = 0; i < InputObj.inventory_stock_detail.length; i++) {
			InputObj.inventory_stock_detail[i].location_id = InputObj.inventory_location_id;
			...
			InputObj.inventory_stock_detail[i].uom_id = InputObj.inventory_stock_detail[i].inventory_uom_id;
			InputObj.inventory_stock_detail[i].sales_uom = InputObj.inventory_stock_detail[i].sales_uom_id;
			InputObj.inventory_stock_detail[i].item_id = InputObj.inventory_stock_detail[i].inv_item_id;
			InputObj.inventory_stock_detail[i].item_code_id = InputObj.inventory_stock_detail[i].inv_item_id;
			...
			InputObj.inventory_stock_detail[i].item_group_id = InputObj.inventory_stock_detail[i].inv_item_group_id;

			InputObj.inventory_stock_detail[i].net_total = InputObj.inventory_stock_detail[i].net_extended_cost;
	...
	let item_grp = [];
	if ($this.state.dn_from === 'PHR') {
		item_grp = _(InputObj.pharmacy_stock_detail).groupBy('item_id').map((row, item_id) => item_id).value();

		for (var k = 0; k < item_grp.length; k++) {
			const pharmacy_stock_detail = _.filter(InputObj.pharmacy_stock_detail, (f) => {
				return f.item_id === parseInt(item_grp[k]);
			});
			let total_recived_qty = _.sumBy(pharmacy_stock_detail, (s) => parseFloat(s.dn_quantity));
			total_recived_qty = total_recived_qty + _.sumBy(pharmacy_stock_detail, (s) => parseFloat(s.free_qty));
			let net_extended_cost = _.sumBy(pharmacy_stock_detail, (s) => parseFloat(s.net_extended_cost));
			let average_cost = (parseFloat(net_extended_cost) / total_recived_qty).toFixed(3);
		...
		item_grp = _(InputObj.inventory_stock_detail).groupBy('item_id').map((row, item_id) => item_id).value();

		for (var l = 0; l < item_grp.length; l++) {
			const inventory_stock_detail = _.filter(InputObj.inventory_stock_detail, (f) => {
				return f.item_id === parseInt(item_grp[l]);
			});

			let total_recived_qty = _.sumBy(inventory_stock_detail, (s) => parseFloat(s.dn_quantity));
			total_recived_qty = total_recived_qty + _.sumBy(inventory_stock_detail, (s) => parseFloat(s.free_qty));
			let net_extended_cost = _.sumBy(inventory_stock_detail, (s) => parseFloat(s.net_extended_cost));
			let average_cost = (parseFloat(net_extended_cost) / total_recived_qty).toFixed(3);
			for (var y = 0; y < InputObj.inventory_stock_detail.length; y++) {
				if (String(InputObj.inventory_stock_detail[y].item_id) === item_grp[l]) {
			...
			if (response.data.success) {
				...
				for (let i = 0; i < data.po_entry_detail.length; i++) {
					if (dn_entry_detail.length === 0) {
						...
						dn_entry_detail = dn_entry_detail.concat(data.po_entry_detail[i].dn_entry_detail);
					}
				}
				data.dn_entry_detail = dn_entry_detail;

				if ($this.props.delivery_note_number !== undefined && $this.props.delivery_note_number.length !== 0) {
