						...
						if (data !== null && data !== undefined) {
							...
							for (let i = 0; i < data.po_entry_detail.length; i++) {
								...
								if ($this.state.po_from === 'PHR') {
									...
									data.po_entry_detail[i].phar_item_group = data.po_entry_detail[i].item_group_id;
									data.po_entry_detail[i].phar_item_id = data.po_entry_detail[i].item_id;

									data.po_entry_detail[i].pharmacy_uom_id = data.po_entry_detail[i].purchase_uom_id;
								} else {
									...
									data.po_entry_detail[i].inv_item_category_id =
										data.po_entry_detail[i].item_category_id;
									data.po_entry_detail[i].inv_item_group_id = data.po_entry_detail[i].item_group_id;
									data.po_entry_detail[i].inv_item_id = data.po_entry_detail[i].item_id;

									data.po_entry_detail[i].inventory_uom_id = data.po_entry_detail[i].purchase_uom_id;
								}

								data.po_entry_detail[i].order_quantity = data.po_entry_detail[i].quantity_authorized;
								data.po_entry_detail[i].total_quantity = data.po_entry_detail[i].quantity_authorized;

								data.po_entry_detail[i].uom_requested_id = data.po_entry_detail[i].item_uom;
								...
								data.po_entry_detail[i].expected_arrival_date = $this.state.expected_date;
								data.po_entry_detail[i].authorize_quantity = 0;
								data.po_entry_detail[i].rejected_quantity = 0;
								data.po_entry_detail[i].tax_percentage = $this.state.tax_percentage;
							...
							let total_tax = Enumerable.from(data.po_entry_detail).sum((s) => parseFloat(s.tax_amount));
							...
							if ($this.state.po_from === 'PHR') {
								data.phar_requisition_id = data.hims_f_pharamcy_material_header_id;
								data.pharmacy_stock_detail = data.po_entry_detail;
							} else {
								data.inv_requisition_id = data.hims_f_inventory_material_header_id;
		...
		onSuccess: (response) => {
			if (response.data.success === true) {
				$this.setState({
					...
					hims_f_procurement_po_header_id: response.data.records.hims_f_procurement_po_header_id,
					saveEnable: true
				});
	...
	$this.props.getPurchaseOrderEntry({
		...
		afterSuccess: (data) => {
			if ($this.props.purchase_number !== undefined && $this.props.purchase_number.length !== 0) {
				...
				for (let i = 0; i < data.po_entry_detail.length; i++) {
					data.po_entry_detail[i].authorize_quantity = data.po_entry_detail[i].total_quantity;
					data.po_entry_detail[i].quantity_outstanding = data.po_entry_detail[i].total_quantity;
					data.po_entry_detail[i].rejected_quantity = 0;
				}
			}
...
const generatePOReceipt = ($this) => {
	algaehApiCall({
		...
		data: {
			report: {
				reportName: $this.state.po_from === 'PHR' ? 'poPharmacyProcurement' : 'poInventoryProcurement',
				reportParams: [
					{
...
const AuthorizePOEntry = ($this) => {
	let stock_detail =
		$this.state.po_from === 'PHR' ? $this.state.pharmacy_stock_detail : $this.state.inventory_stock_detail;
