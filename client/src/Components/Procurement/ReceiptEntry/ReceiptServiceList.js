...
export default class ReceiptServiceList extends Component {
    ...
    deleteSalesDetail(context, row) {
        let receipt_entry_detail_services = this.state.receipt_entry_detail_services;
        ...
        if (receipt_entry_detail_services.length === 0) {
            ...
            const sub_total = _.sumBy(receipt_entry_detail_services, s => parseFloat(s.sub_total));
            const discount_amount = _.sumBy(receipt_entry_detail_services, s => parseFloat(s.discount_amount));
            const net_total = _.sumBy(receipt_entry_detail_services, s => parseFloat(s.net_total));
            const total_tax = _.sumBy(receipt_entry_detail_services, s => parseFloat(s.total_tax));
            const net_payable = _.sumBy(receipt_entry_detail_services, s => parseFloat(s.net_payable));
