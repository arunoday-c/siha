...
import { LocationchangeTexts, dateFormater, texthandle, poforhandle } from './PurchaseOrderListEvent';

import { AlgaehDataGrid, AlgaehLabel, AlagehAutoComplete } from '../../Wrapper/algaehWrapper';
...
class PurchaseOrderList extends Component {
	...
	render() {
		const _mainStore = Enumerable.from(this.props.polocations).where((w) => w.location_type === 'WH').toArray();
		return (
			<React.Fragment>
				<div className="hptl-phase1-requisition-list-form">
					<BreadCrumb
						title={<AlgaehLabel label={{ forceLabel: 'Purchase Auth List', align: 'ltr' }} />}
						...
						pageNavPath={[
							...
							{
								pageName: <AlgaehLabel label={{ forceLabel: 'Purchase Auth List', align: 'ltr' }} />
							}
						]}
					/>
								...
								<div className="portlet-body" id="purchaseOrderListCntr">
									<AlgaehDataGrid
										...
										columns={[
											...
											{
												fieldName: 'po_date',
												label: <AlgaehLabel label={{ forceLabel: 'PO Date' }} />,
												displayTemplate: (row) => {
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(PurchaseOrderList));
