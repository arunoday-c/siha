...
export default memo(function TopSelection(props) {
  ...
  useEffect(() => {
    if (appointmentDate && appointmentDate !== "") {
      setActiveDateHeader(moment(appointmentDate));
      setSelectedDate(moment(appointmentDate));
    } else {
      setSelectedDate(moment(appointmentDate));
    }
  }, [appointmentDate]);
  useEffect(() => {
    ...
    if (appointmentDate && appointmentDate !== "" && isChangedDate === false) {
      ADH = moment(appointmentDate);
    }
    if (ADH) {
  ...
  function onChangeSelectDate(e) {
    ...
    setAppointmentDate(e.currentTarget?.dataset?.date);
    history.push(
      pathName +
        `?appointmentDate=${moment(e.currentTarget?.dataset?.date).format(
          "YYYY-MM-DD"
        )}`
    );
  }
